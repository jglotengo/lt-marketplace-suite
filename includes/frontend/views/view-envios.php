...
<div class="ltms-view-section">
    ...
    <div class="ltms-view-header" style="margin-bottom:24px;">
        <h2 style="margin:0;font-size:1.35rem;font-weight:700;color:#111827;">
            🚚 <?php esc_html_e( 'Relaciones de Envíos', 'ltms' ); ?>
        </h2>
        <p style="margin:4px 0 0;font-size:0.85rem;color:#6b7280;">
            <?php esc_html_e( 'Agrupa tus guías en un manifiesto de despacho para entregar a la transportadora.', 'ltms' ); ?>
        </p>
    </div>
    ...
    <div id="ltms-envios-delete-modal"
        ...
        <div style="background:#fff;border-radius:12px;padding:28px;max-width:400px;width:90%;box-shadow:0 20px 60px rgba(0,0,0,0.15);">
            <h4 style="margin:0 0 12px;font-size:1rem;color:#111827;">⚠️ <?php esc_html_e( 'Eliminar relación', 'ltms' ); ?></h4>
...
(function($) {
                ...
                rows += `<tr style="border-bottom:1px solid #f3f4f6;">
                    ...
                    <td style="padding:10px 16px;text-align:center;">
                        <div style="display:flex;gap:6px;justify-content:center;flex-wrap:wrap;">
                            ${r.rutaimpresion ? `<a href="${r.rutaimpresion}" target="_blank" style="padding:4px 10px;background:#eff6ff;color:#1d4ed8;border:1px solid #bfdbfe;border-radius:5px;font-size:0.75rem;text-decoration:none;white-space:nowrap;">🖨 Imprimir</a>` : ''}
    ...
    $('#ltms-envios-create-btn').on('click', function() {
        ...
        $.post(ajaxUrl, { action: 'ltms_vendor_create_relation', nonce, transportadora: carrier, guias }, function(res) {
            ...
            const printBtn = d.rutaimpresion
                ? `<a href="${d.rutaimpresion}" target="_blank" style="display:inline-block;margin-top:10px;padding:7px 14px;background:#1d4ed8;color:#fff;border-radius:6px;text-decoration:none;font-size:0.82rem;">🖨 Imprimir manifiesto</a>`
                : '';
    ...
    function searchRecipients(param) {
        ...
        $.get(ajaxUrl, { action: 'ltms_aveonline_search_recipients', nonce, param }, function(res) {
            ...
            res.data.destinatarios.forEach(d => {
                html += `<div class="ltms-recipient-item" style="padding:8px 10px;background:#fff;border:1px solid #e0f2fe;border-radius:6px;cursor:pointer;font-size:0.8rem;" data-nombre="${d.nombre}" data-direccion="${d.direccion}" data-telefono="${d.telefono}">
                    <strong>${d.nombre}</strong> — ${d.direccion} <span style="color:#9ca3af;font-size:0.72rem;">${d.telefono || ''}</span>
                </div>`;
            });
            html += '</div>';
            $r.html(html).show();
    ...
    $(document).on('click', '.ltms-envios-delete-btn', function() {
        deleteTarget = $(this).data('relacion');
        $('#ltms-envios-delete-msg').text(`¿Eliminar la relación ${deleteTarget} en Aveonline? Esta acción no se puede deshacer.`);
