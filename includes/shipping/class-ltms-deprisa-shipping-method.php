...
class LTMS_Deprisa_Shipping_Method extends WC_Shipping_Method {
        ...
        public function calculate_shipping( $package = [] ): void {
                ...
                // SEC-6 FIX (v2.9.25): descifrar password si está cifrada (v1: prefix).
        ...
        private function cotizar_en_deprisa( string $ciudad_destino, float $kilos, float $valor_declarado ): ?float {
                ...
                try {
                        // SEC-6 FIX: descifrar password antes de pasar al API client.
                        $pw_raw = get_option( 'ltms_deprisa_password' );
                        $pw_dec = ( str_starts_with( $pw_raw, 'v1:' ) && class_exists( 'LTMS_Core_Security' ) )
                                ? LTMS_Core_Security::decrypt( $pw_raw )
                                : $pw_raw;
                        ...
                        $params = [
                                ...
                                'importeValorDeclarado' => max( (float) apply_filters( 'ltms_deprisa_min_declared_value', 4500.0, 'COP' ), $valor_declarado ),
                                'tipoMoneda'            => 'COP',
                        ];
