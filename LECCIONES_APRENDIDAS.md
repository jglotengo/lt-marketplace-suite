...
### Lección 27.1: Inconsistencia meta keys + cron huérfano + bypass de fail-closed default

**Contexto:** AUDIT-BUSINESS-CICLO27, auditando `class-ltms-zapsign-manager.php` (556L reales, marcado CRÍTICO en AGENTS.md "Revisión como último filtro"). El módulo tenía ya_multiple fixes previos (M-ZAPSIGN-GAP-01, BC-01, ZS-1, ZS-2, SEC-4, EXCMSG-FIX, INTEGRATIONS-AUDIT P0/P1) — la auditoría a fondo reveló que **los fixes previos crearon superficie para bugs nuevos** que pasaron desapercibidos por 2 razones: (a) el webhook handler y el manager estaban desacoplados en sus convenciones de meta keys, y (b) el cron `ltms_zapsign_poll_pending` estaba programado por el activator sin que nadie registrara handler — lo que enmascaraba que `get_contract_status()` era código huérfano.

**Bug ZS-MGR-008 (P1): Inconsistencia meta keys publicas vs privadas entre webhook y manager.** El webhook de ZapSign, al recibir `doc_signed`, actualizaba `ltms_kyc_status='approved'` (public, sin underscore) + `_ltms_zapsign_doc_token` / `_ltms_zapsign_signed_at` (privadas, con underscore). Pero `class-ltms-zapsign-manager.php` y `class-ltms-compliance-guardian.php` esperaban `ltms_contract_status` / `ltms_contract_signed_at` (publicas, sin underscore) — esas metas NUNCA se actualizaban a 'signed' desde el webhook. El contract quedaba permanentemente 'pending' en user_meta publica aunque firmado. **Causa raíz:** cada desarrollador que añadió un meta key en su isula Usó su propia convención (con/sin underscore) sin un schema central. El fix ZS-MGR-008 armoniza el webhook con el del manager: ahora ambos actualizan el mismo set de metas publicas + el `ltms_contract_status_verified_at` (que el manager usa para el rate-limit 24h del ZS-2 FIX).

**Bug ZS-MGR-007 (P1): Bypass de un fix previo (ZS-2) por `default => $cached_status ?: 'unknown'` en match.** El ZS-2 FIX intentaba detectar contratos retractados post-firma (cancelled/refused/expired). Pero el `match` solo cubría 6 estados conocidos de ZapSign + un `default` que dejaba el cache previo. Si ZapSign introducía un estado nuevo en su API (ej. `archived`, `legal_hold`, `pending_review`), el sistema decía "no lo conozco, dejo el cache signed" — **re-abriendo el bypass que ZS-2 pretendía cerrar**. **Causa raíz:** el `match` enum fue definido pensando en los estados conocidos, no en los desconocidos. **Fix:** `default => 'unknown'` (fail-closed). Regla: SIEMPRE que un `match` este diseñado para distinguir "lo que apruebo" vs "lo que no", el default debe ser "lo que no apruebo" (fail-closed), no "lo que decía el cache" (que puede ser el valor approved).

**Bug ZS-MGR-008b (P1): Cron huérfano + método público sin callers = sistema muerto.** El activator (`class-ltms-activator.php:613`) programa `wp_schedule_event(time()+1800, 'hourly', 'ltms_zapsign_poll_pending')` — el cron disparaba cada hora al aire, sin handler. Y `LTMS_ZapSign_Manager::get_contract_status()` era un método público **sin callers** en todo el codebase (verificado via grep en `includes/`). El ZS-2 FIX era buena lógica muerta — el código que re-verificaba contratos signed nunca corría. **Causa raíz:** falta de trazabilidad entre `wp_schedule_event` y `add_action` — ambos viven en archivos distintos (activator.php vs manager.php) sin un schema central que los vincule. **Fix:** registrar `add_action('ltms_zapsign_poll_pending', [__CLASS__, 'poll_pending_contracts'])` en `init()` del manager, con un handler que delega a `get_contract_status()` (que ya tenía toda la lógica de re-verificación).

**Reglas preventivas:**

1. **Trazabilidad cron ↔ handler.** Cada `wp_schedule_event($ts, $rec, $hook)` debe tener su correspondiente `add_action($hook, ...)` registrada en el `init()` de una clase del plugin. Sin eso, el cron dispara al vacío (silenciosamente, sin error). **Regla:** antes de declarar un ciclo cerrado, `grep -r "wp_schedule_event" .` y para cada `$hook` encontrado, verificar que existe `add_action($hook, ...)` en algún archivo del plugin. Si no existe, es cron huérfano.

2. **Métodos públicos sin callers son código muerto.** `grep` con el nombre del método en todo `includes/` + `admin/` + `templates/`. Si el único match es la declaración del propio método, nadie lo invoca. AGENTS.md "no dejes código muerto "por si acaso"" aplica: o eliminar el método, o conectarlo a un caller. En C27, ZS-MGR-008b conectó `get_contract_status()` al cron huérfano, resolviendo ambos problemas de un golpe.

3. **`match` enum debe ser fail-closed.** Cuando un `match` asocia estados remotos a estados locales, y existe un estado local equivalente a "approved" (como `'signed'`), el `default` NO debe ser `$cached_status` (que puede ser el approved) sino un valor neutro / pendiente-revalidación (`'unknown'`). Estados nuevos en la API externa pueden introducirse sin aviso; el default fall-closed asegura que estos estados no preserven un approved legacy.

4. **Schema central de meta keys.** Cada nueva meta key introducida en user_meta debe definirse en un único sitio del plugin (constante de clase, schema central, o docstring del header de la clase). El bug ZS-MGR-008 nació de dos convenciones (`_ltms_zapsign_*` con underscore vs `ltms_contract_*` sin underscore) en dos archivos distintos sin un contracto. Regla: cuando se añade un meta key nueva, `grep` en TODO el codebase para confirmar que los lectores esperan el key exacto (no una variante con underscore).

5. **Fixes previos crean superficie para bugs nuevos.** Cada fix nuevo puede introducir una nueva invariante (ej. el ZS-2 FIX introdujo `ltms_contract_status_verified_at` como rate-limit key) que otros componentes deben compensar. La auditoría a fondo del módulo DESPUÉS de un fix previo revela si esa invariante está siendo respetada en TODOS los callers actualizadores. En C27, el webhook no estaba inicializando `ltms_contract_status_verified_at` — el rate-limit del ZS-2 FIX dependía de un key que solo el manager escribía.

6. **Módulo CRITICOZAPSIGN exige 2a revisión del diff.** AGENTS.md "Revisión como último filtro" marca ZapSign junto con wallet/comisiones/payouts/KYC/SAGRILAFT/Backblaze/gateways de pago. La 2a revisión de C27 (subagente general) confirmó: no P0/P1 adicionales, defenses previas intactas, race webhook↔cron resuelto. **Regla:** aunque el primer autor esté seguro, el fix de un módulo crítico SIEMPRE requiere una segunda lectura de alguien sin el contexto de implementación — es el último filtro antes del deploy.


### Lección 28.1: `check_ajax_referer(..., false)` sin `wp_send_json_error` dentro = bypass CSRF silencioso en endpoints `nopriv`

**Contexto:** AUDIT-BUSINESS-CICLO28, auditando `class-ltms-compliance-guardian.php` (826L post-fix). Módulo de compliance regulatorio (Ley 1581/2012 Colombia ARCO + LFPDPPP México PLD + RGPD Meta CAPI M3/M4/M10/M14 + GDPR). NO crítico estricto AGENTS.md pero SI compliance legal — 2a revisión aplicada igual por Lección 27.1 regla #6. La auditoría reveló que este módulo es un **patrón recurrente en el plugin**: el anti-patrón `check_ajax_referer(..., false)` apareció EN 3 ARCHIVOS DISTINTOS — C26 lo detectó como TB-007 en `traffic-booster.php:475` (declarado P2 backlog), C28 lo encuentra de nuevo aquí en `compliance-guardian.php:807` (escalado a P0 por contexto compliance), y la 2a revisión de C28 detectó dos casos más en `sales-booster.php:805, 833` (candidato C29). El anti-patrón se había extendido por el plugin sin que ningún ciclo previo lo detectara como sistemático.

**Bug CG-001 (P0): `check_ajax_referer(..., false)` ignoraba el fallo del nonce.** El endpoint `wp_ajax_nopriv_ltms_cookie_consent` es accesible por cualquier visitante (sin login). El handler llamaba `check_ajax_referer( 'ltms_ux_nonce', 'nonce', false )` — el tercer parámetro `$die=false` evita el `wp_die()` automático de WP. El bloque `if ( ! check_ajax_referer(...) )` contenía SOLO un comentario ("For backward compat with cached pages that may not have nonce, still process but don't log consent"). El endpoint PROCESABA IGUAL: seteaba cookie consent level a `full`, logueaba `cookie_full` a `lt_consent_log`, e inhabilitaba todos los gates de consent M3/M10/M14. Cualquier site podía embed un `<form method="POST" action="/wp-admin/admin-ajax.php">` con `action=ltms_cookie_consent&nonce=falso&level=full` y el endpoint forzaba el consent `full` silenciosamente — bypass completo del consent gating. **Violación Ley 1581 art. 9** (consentimiento libre/previo/expreso/informado — CSRF lo viola todo) + **RGPD art. 7** + **Meta policy M3** (Pixel sin consent explícito). El comentario "backward compat with cached pages" intentaba justificar el bypass — pero una página cacheada NO gana bypass de compliance: el cache es problema del front (banner JS debe entregar nonce fresco via `wp_localize_script`), no del endpoint. Fix: `wp_send_json_error( [...], 403 )` dentro del bloque fail-closed.

**Bug CG-002 (P1): Invariante transversal `get_client_ip_safe()` no respetado en CAPI.** Las dos funciones `build_capi_user_data` (order flow) y `build_capi_user_data_from_session` (cart flow) usaban `LTMS_Utils::get_ip()` en lugar de `LTMS_Core_Security::get_client_ip_safe()`. La Lección 25.1 establece get_client_ip_safe() como fuente única de IP (sanitiza spoofing-resistente — solo confía en XFF si REMOTE_ADDR está en ltms_trusted_proxies). `LTMS_Utils::get_ip()` confía en cualquier header XFF sin validación — spoofable. Aunque el IP se enviaba a Meta (no se persistía en DB), la divergencia con el invariante transversal endurece riesgo y diverge entre gateways. La 2a revisión confirmó que `class-ltms-deposit.php:164` y `class-ltms-wallet.php:606` todavía usan `LTMS_Utils::get_ip()` — backlog C29 futuro (test C28 `test_ltms_utils_get_ip_method_still_exists` documenta esta dependencia).

**Bug CG-003 (P1): ARCO Cancelación no eliminaba las oposiciones del propio módulo.** El endpoint DELETE `/ltms/v1/privacy/cancel` eliminaba 26 metas PII pero NO incluía las 5 metas de oposición/option del propio compliance-guardian: `ltms_opposition_{marketing,profiling,data_sharing,automated_decisions}` (escritas por `arco_oppose`) y `ltms_meta_data_opt_out` (escrita por `save_meta_opt_out`). El usuario que pide "cancel" asume que NADA de su voluntad permanece. Ley 1581 art. 8 lit. e y LFPDPPP art. 25 exigen Cancelación efectiva. Las entries en `lt_consent_log` (evidencia histórica) SÍ se retienen por obligación fiscal (ET art. 632 / LISR art. 30), pero el `user_meta` `ltms_opposition_*` es la oposición ACTUAL — en cuenta cerrada no recibirá marketing, retenerla es basura sin propósito válido y viola el principio de minimización (Ley 1581 art. 4 lit. c). Lo mismo aplica a `ltms_meta_data_opt_out` (M14): cuenta cerrada no enviará eventos CAPI — retener el flag no aporta nada. Fix: añadir las 5 metas al array `$pii_keys` que `arco_cancel` recorre.

**Reglas preventivas:**

1. **`check_ajax_referer(..., false)` exige `wp_send_json_error` dentro del bloque.** El tercer parámetro `$die=false` se usa para evitar el `wp_die()` automático de WP y poder retornar un JSON controlado — pero NO significa "ignorar el fallo". Si el bloque `if ( ! check_ajax_referer(...) )` queda vacío o solo con comentario, el endpoint procesa la request CSRF igual. Regla: TODO `check_ajax_referer(..., false)` debe ir seguido de `wp_send_json_error( [...], 4xx )` dentro del bloque `if`. Grep anti-patrón: `grep -rn "check_ajax_referer.*false" includes/` — cada match debe tener `wp_send_json_error` en las siguientes 3 líneas. Apareció en 4 sitios del plugin: traffic-booster:475 (TB-007 C26), compliance-guardian:807 (CG-001 C28, fixeado P0), sales-booster:805 + 833 (candidato C29). El anti-patrón se había extendido sin que ciclos previos lo detectaran como sistemático.

2. **Endpoint `wp_ajax_nopriv_*` (sin login) + compliance = SIEMPRE P0 cuando bypass nonce.** Los endpoints `nopriv` son accesibles por cualquier visitante sin sesión. Si además tocan consent/privacidad/compliance regulatorio (Ley 1581, RGPD, LFPDPPP, Meta CAPI M3/M10/M14), un bypass de nonce forja consentimiento del usuario — violación directa del art. 9 Ley 1581 (consentimiento libre/previo/expreso/informado). Misma clase de bug en un endpoint `nopriv` de marketing (TB-107 traffic-booster) era P2 — en un endpoint `nopriv` de compliance regulatorio es P0. Regla: al auditar endpoints `nopriv`, el severity se escala automáticamente si el handler toca compliance/privacy/consent/PII/KYC.

3. **Cancelación ARCO efectiva exige limpieza exhaustiva del `user_meta` activo.** El endpoint DELETE `/ltms/v1/privacy/cancel` debe eliminar TODAS las metas que representan "voluntad activa del usuario" (consentimientos, oposiciones, opt-outs) — no solo las PII evidentes (nombre, email, KYC). La evidencia HISTÓRICA (en `lt_consent_log` u otra tabla de auditoría) se retiene por obligación fiscal, pero el estado ACTIVO (`user_meta`) debe desaparecer completamente. Regla: cada función que escribe `update_user_meta( $user_id, 'ltms_*', ... )` debe tener una entry correspondiente en el array `$pii_keys` que `arco_cancel` recorre. Verificar con grep: `grep -n "update_user_meta.*'ltms_" includes/business/class-ltms-compliance-guardian.php` — cada key debe aparecer en `$pii_keys` o ser justificada como exención (ej. `ltms_kyc_*` ya cubiertos).

4. **Invariante transversal `get_client_ip_safe()` requiere grep periódico.** Lección 25.1 (C25) cerró la divergencia en 5 webhook handlers. Lección 26.1 (C26) extends a business/traffic-booster. Lección 27.1 (C27) extends a zapsign webhook. Lección 28.1 (C28) extends a compliance-guardian CAPI. Pero 2 ocurrencias siguen sin migrar: `class-ltms-deposit.php:164` y `class-ltms-wallet.php:606` (backlog C29). Regla: cada ciclo de auditoría debe ejecutar `grep -rn "LTMS_Utils::get_ip" includes/` y verificar que TODAS las ocurrencias están justificadas (comentario de backlog es válido; uso activo sin comentario es debt técnico).

5. **El anti-patrón `check_ajax_referer(..., false)` parecía aislado pero era sistemático.** C26 detectó 1 caso (TB-007) y lo declaró P2 backlog. C28 detectó otro caso (CG-001) en contexto compliance y lo escaló a P0 — y durante la 2a revisión descubrió 2 casos más en `sales-booster.php`. La lección: un anti-patrón detectado en 1 archivo puede estar en N archivos más. Regla: cuando se detecta un anti-patrón de seguridad (non-zero positivo en grep), grep TODO el codebase INMEDIATAMENTE — no esperar al próximo ciclo. El costo de grep es 1 comando; el costo de esperar es otro P0 en producción.

6. **Contexto regulatorio escala el severity sin modificar el patrón del bug.** TB-007 (traffic-booster, marketing newsletter) era P2 porque toca infra de marketing no-financiero. CG-001 (compliance-guardian, consentimiento Ley 1581) es P0 porque toca compliance regulatorio. El MISMO anti-patrón (`check_ajax_referer(..., false)` sin `wp_send_json_error`). La diferencia de severity viene del DAÑO POTENCIAL: un bypass de consent gating viola el art. 9 Ley 1581 (sanción SIC Colombia). Regla: al clasificar severity de un hallazgo, considerar (a) anti-patrón técnico + (b) dominio regulatorio del módulo. Mismo anti-patrón en módulo crítico/compliance = P0; en módulo admin interno = P2.


### Lección 29.1: Endpoint `wp_ajax_*` (con login) sin `check_ajax_referer` = CSRF tan grave como en `nopriv` + la 2a revisión detecta lo que la 1a auditoria no vio

**Contexto:** AUDIT-BUSINESS-CICLO29, auditando `class-ltms-sales-booster.php` (789L pre-fix, 925L post-fix) — modulo de marketing (SB-1 a SB-5). NO es modulo CRITICO AGENTS.md y NO toca compliance regulatorio (a diferencia de C28). 2a revision opcional aplicada anyway (Leccion 27.1 regla #6 no obliga para marketing, pero el modulo tocaba surface PHP CSRF + UX vendor). La 1a auditoria detecto SB-001 P0 (CSRF en `ajax_subscribe_push`) + SB-002 P1 (feature rota JS sin nonce). La 2a revision subagente general **DEVOLVIO NO APROBADO** marcando SB-007 P1 que la 1a auditoria no detecto (`$_SERVER['REMOTE_ADDR']` crudo en `session_id` del viewer count, violando invariante transversal Leccion 25.1). Estaesion es la primera vez en el loop donde la 2a revisión obligatoria cambio el veredicto: el subagente identifice un P1 real que faltaba.

**Bug SB-001 (P0): `wp_ajax_ltms_subscribe_push` SIN `check_ajax_referer` (no es `nopriv` pero CSRF aplica igual).** El endpoint `wp_ajax_ltms_subscribe_push` (requiere login via cookie de sesion) NO tenia NINGUN `check_ajax_referer`. Anti-patron PEOR que CG-001 C28: alla el nonce existia pero se ignoraba (check_ajax_referer con false sin wp_send_json_error); aqui el nonce brillaba por su ausencia total. Cualquier site podia embed `<form method="POST" action="/wp-admin/admin-ajax.php">` con `action=ltms_subscribe_push&endpoint=https://atacante.com/...` y la request se procesaba porque la cookie de sesion del usuario logueado viajaba con la request (CSRF clasico). El usuario quedaba subscrito a push notifications desde endpoint controlado por atacante — superficie de phishing + exfiltracion de datos. **Mito roto:** CSRF no aplica solo a endpoints `nopriv`. Cualquier endpoint `wp_ajax_*` (con login) es vulnerable a CSRF via cookie de sesion + form/embed de tercer site. El nonce es el unico mecanismo anti-CSRF en WP — sin el, la autenticacion por cookie NO protege.

**Bug SB-002 (P1): JS front NO enviaba nonce aunque el backend lo exigia — feature funcionalmente ROTA en runtime.** El handler `ajax_track_product_view` tenia `check_ajax_referer('ltms_ux_nonce', 'nonce', false)` fail-closed (v2.9.100 SEC-8 FIX previo). Pero el JS `render_social_proof_container` hacia `$.post(ajaxurl, { action: 'ltms_track_product_view', product_id: productId })` SIN enviar el nonce. Resultado: todas las requests retornaban 403 — la feature de viewer count era no-operativa en producción. **Causa raíz:** el SEC-8 FIX aplico solo el backend, no sincronizo el JS front. Un fix "front-end" (o falta de el) invisibiliza el bug — el developer que agrego el SEC-8 nonce check no noto que el JS no lia el nonce. **Regla:** cuando se agrega `check_ajax_referer` a un handler, SIEMPRE verificar que el JS que lo llama envie el nonce. Test funcional end-to-end (no solo source-check del backend) detecta esto.

**Bug SB-007 (P1, 2a revision): `$_SERVER['REMOTE_ADDR']` crudo en `session_id` viola invariante transversal Leccion 25.1.** El handler `ajax_track_product_view` usaba `$_SERVER['REMOTE_ADDR'] ?? '0.0.0.0'` como fallback de `session_id` del viewer count. **Viola Leccion 25.1** (`get_client_ip_safe()` fuente unica de IP, spoofing-resistente). Atras de reverse proxy (SiteGround) todas las requests comparten la IP del proxy — todos los viewers se unify en 1 (malogra la feature, pero no es critico). Mas serio: spoofable via X-Forwarded-For sin validar trusted_proxies — un atacante podria inflar el contador enviando XFF distinto por request, neutralizando SEC-8 FIX que protegia CSRF pero no IP-tracking. **Este bug NO fue detectado por la 1a auditoria.** La 2a revision subagente general lo encontro en `class-ltms-sales-booster.php:856`. La 1a auditoria se concentro en el anti-patron sistematico CG-001 (CSRF en `nopriv`) y paso por alto la invariante transversal IP. La 2a revision explicitamente busco invariantes transversales en el modulo tocado C29 — algo que la 1a auditoria no explicito buscar.

**Reglas preventivas:**

1. **Endpoint `wp_ajax_*` (con login) tambien requiere nonce.** Mito comun: "CSRF solo afecta endpoints `nopriv`". FALSO. Cualquier endpoint con cookie de sesion es vulnerable a CSRF — cualquier site puede embed un form que viaja con la cookie del usuario. El `check_ajax_referer` es el unico mecanismo anti-CSRF en WP, sin distincion entre `nopriv` y `wp_ajax_*`. Regla: TODO endpoint `wp_ajax_*` (con o sin login) debe tener `check_ajax_referer`. La autenticacion por cookie NO protege contra CSRF — el nonce si. Grep anti-patron: `grep -rn "wp_ajax_" includes/ | grep -v nopriv` y para cada handler verificado que existe `check_ajax_referer`.

2. **Sincronizar front-end JS cuando se agrega `check_ajax_referer` al backend.** Un fix backend-only que agrega nonce check puede romper feature sin que nadie lo note — el JS sigue llamando sin nonce, todas las requests fallan 403, feature queda no-operativa en runtime. El SEC-8 FIX agrego nonce check al handler `ajax_track_product_view` en v2.9.100 pero el JS `render_social_proof_container` nunca se sincronizo — la feature roto probablemente desde v2.9.100 hasta C29 (meses). Regla: cuando un fix agrega `check_ajax_referer` a un handler, INMEDIATAMENTE verificar que el JS que lo llama envie el nonce. Si no, el fix es incomplete — feature queda rota.

3. **La 2a revision detecta invariantes transversales que la 1a auditoria puede pasar por alto.** La 1a auditoria se concentro en el anti-patron sistemico CG-001 (CSRF en `nopriv`) y no explicito buscar la invariante transversal IP en el modulo C29. La 2a revision subagente general, al no estar contaminado por el contexto de la 1a auditoria, explicito buscar invariantes transversales y encontro SB-007. **Regla madre actualizada:** la 2a revision no es opcional — incluso en modulos no criticos (sales-booster es marketing), puede detectar P1 que la 1a auditoria no detecto. La regla "2a revision obligatoria para modulos criticos AGENTS.md" se relaja a "opcional" en modulos no criticos, pero el costo de omitirla es detectar el hallazgo en el SIGUIENTE ciclo (o en produccion). C29 es prueba empirica: el costo de hacer la 2a revision opcional anyway es 1 subagente general (~5 min de tokens), el beneficio es 1 P1 detectado y fixeado en el mismo commit (no pasando a backlog).

4. **La invariante transversal `get_client_ip_safe()` requiere grep en CADA modulo auditado, no solo en cross-checks.** El test C29 inicial audita `get_client_ip_safe` en 5 webhooks + traffic-booster + xcover + zapsign — pero NO en sales-booster (el modulo C29). El gap fue detectado por la 2a revision. Regla: el test AuditCicloN debe SIEMPRE incluir cross-check del invariante IP en el modulo auditado en ese ciclo, no solo en modulos previos. Cross-checks transversales no son retroactivos — son simultaneos a la auditoria del modulo actual.

5. **`absint()` es robusto vs backslashes pero `wp_unslash` sigue siendo regla.** El 2a revisor marco como P2 SB-006: `absint($_POST['product_id'] ?? 0)` sin `wp_unslash`. `absint` castea a int (rechaza strings no-numericos), pero AGENTS.md dice "usa wp_unslash sobre $_POST". La regla no es sobre robustez — es sobre consistencia. Regla: TODO input `$_POST[...]` debe pasar por `wp_unslash` antes de sanitizar, sin importar el sanitizer siguiente. La regla es arquitectural, no funcional.

### Lección 30.1: `accepted_args` del hook vs args del `do_action` — idempotency_key colisión silenciosa en débitos financieros + mutar estado pre-operación que puede fallar

**Contexto:** AUDIT-BUSINESS-CICLO30, auditando `class-ltms-fiscal-annual-close.php` (506L post-fix) — modulo compliance fiscal-financiero (GMF 4x1000 Colombia/DIAN + cierre anual + PAC CFDI México). Modulo CRITICO AGENTS.md "Revisión como ultimo filtro" → 2a revisión OBLIGATORIA (Leccion 27.1 regla #6). 1a auditoria detecto FAC-001 P0 (idem_key colisión mensual) y FAC-002 P1 (accumulated pre-debit fails silently). 2a revisión subagente general devolvió APROBADO PARA COMMIT en primera vuelta (no P0/P1 nuevos). Ambos bugs latentes desde la implementación original LF-3 (v2.9.11 — versión del archivo cabecera), nunca detectados en ciclos previos porque el modulo no estaba en el mapeo business/ de C3-C19/C26-C29.

**Bug FAC-001 (P0): `accepted_args` del hook < args del `do_action` — `$payout_id` descartado → idempotency_key colisiona mensualmente.** El callback `calculate_gmf_on_payout` estaba registrado con `add_action('ltms_payout_completed', [__CLASS__, 'calculate_gmf_on_payout'], 10, 2)` — `accepted_args=2`. Pero el action se dispara con **3 args** desde `payout-scheduler.php:683` (`do_action('ltms_payout_completed', $vendor_id, (float) $amount, $payout_id)`) y `openpay-webhook-handler.php:349` (`do_action('ltms_payout_completed', $vendor_id, (float) $payout['amount'], $payout_id)`). WordPress silently descarta los args excedentes cuando `accepted_args` es menor que los pasados al `do_action` — el `$payout_id` se caía al limbo. Sin `$payout_id`, la `idempotency_key` se construía como `sprintf('gmf_payout_v%d_%s', $vendor_id, $month)` — **MISMA key para todos los retiros del mes del mismo vendor**. La `LTMS_Business_Wallet::execute_transaction()` (linea 380-396 en class-ltms-wallet.php, invariante WL-CRASH-2) hace `SELECT id FROM lt_wallet_transactions WHERE reference = %s`, y si existe, retorna `existing_tx_id` **sin re-ejecutar la operation** (previene doble-crash-transacción). Resultado: el primer débito GMF del mes persistía `reference = "gmf_payout_v42_2026-08"` en `lt_wallet_transactions`. El segundo payout del mismo vendor en el mismo mes encontraba esa `reference`, hacia skip silencioso (log info `WALLET_IDEMPOTENT_SKIP`) — **el GMF 4x1000 del segundo+ retiro NUNCA se debitaba**. Agente retenedor perdía GMF adeudado a la DIAN sobre todos los retiros del mes salvo el primero. Bug fiscal-financiero serio. Latente desde LF-3 (4+ ciclos). **Mito roto:** "idempotency_key colisión es rara/inofensiva". FALSO — cuando la key se construye con granularidad incorrecta (mes vs payout), colisiona masivamente y resetea el débito al primer uso del periodo.

**Bug FAC-002 (P1): mutar estado persistente (acumulado mensual) ANTES de la operación que puede fallar.** `update_user_meta($monthly_gmf_key, $accumulated + $amount)` se ejecutaba en línea 77 (ANTES del try/catch alrededor de `Wallet::debit()`). Si `Wallet::debit()` lanzaba `InvalidArgumentException` por saldo insuficiente (linea 175-183 en class-ltms-wallet.php), el acumulado mensual de exención (350 UVT, ~$18.4M COP 2026) quedaba inflado sin que el débito GMF se realizara → el siguiente retiro del mes veía el accumulated ya incrementado y calculaba la base imponible sobre una fracción erronea → pérdida silenciosa de GMF fiscal. **Causa raíz:** patrón "actualizar libro, después hacer operación" — si la operación falla, el libro queda inconsistente. El patrón correcto en operaciones financieras es "hacer operación, después actualizar libro solo si éxito". Igual que un `INSERT` en DB con idempotency_key como `reference`UITaed, el `update_user_meta` debe ser POST-success, no PRE-attempt.

**Reglas preventivas:**

1. **`accepted_args` del `add_action` debe ser igual o mayor que los args del `do_action` que lo dispara.** WordPress NO lanza warning si el callback acepta menos args de los que se disparan — los descarta silenciosamente. Esto es especialmente insidioso cuando el callback construye una clave única (idempotency, cache key, uniqid) basada en args del action. Si la clave UNIQUE se construye sin uno de los args (p.ej. payout_id), colisiona entre todas las invocaciones que comparten los otros args (vendor_id + month). Regla: auditar cada `add_action($hook, $callback, $priority, $accepted_args)` contra TODOS los `do_action($hook, ...)` del plugin — `accepted_args` debe ser >=$max(args) de todos los fire sites. Grep anti-patron: `grep -rn "do_action( 'ltms_.*'" includes/` para cada action, list los args; luego `grep -rn "add_action( 'ltms_.*'" includes/` para cada subscription, verificado `accepted_args` cubre.

2. **`idempotency_key` debe ser única por unidad de operación atómica, no por período.** `sprintf('gmf_payout_v%d_%s', $vendor_id, $month)` agrupa por mes — útil para un "resumen mensual" pero NO para una operación por-payout. Cada payout debe tener su propia idempotency_key (`sprintf('gmf_payout_v%d', $payout_id)`) porque el débito GMF se hace por payout (uno a uno), no una vez por mes. Regla: cuando una operación atómica (un débito, una llamada a API externa, un INSERT) tiene un identificador PK único (`$payout_id`, `$order_id`, `$tx_id`), la `idempotency_key` debe incluir ese PK — no agrupaconcatenar por período (`mes`, `dia`), porque múltiples operaciones del mismo PK comparten gruppo y colisionan. La `reference` en la tabla transaccional es la equality de idempotency — debe ser única por unidad operativa.

3. **Mutar estado persistente `update_user_meta`/`update_post_meta`/`update_option` DESPUÉS de la operación que puede fallar.** No ANTES. Patrón anti-financiero: `update_meta(acumulado + amount); try { debit() } catch { log() }` — si debit falla, accumulated queda inflado. Patrón correcto: `try { debit(); update_meta(acumulado + amount); } catch { return; }` — accumulated solo se actualiza si debit exitoso. Lo mismo aplica a `update_option` de caches/resumenes/indexes que se derivan de operaciones que pueden fallar (WP failures, network errors, exceptions). Regla: el orden es "operación atómica → mutar libro"; nunca "mutar libro → operación atómica".

4. **When auditar módulos con hooks a acciones financieras (`ltms_payout_completed`, `ltms_order_paid`, `ltms_commission_*`), SIEMPRE grep `do_action` fire sites Y `add_action` subscribers.** Los fire sites determinan cuantos args se pasan; los subscribers determinan cuantos args aceptan. Si el subscriber construye una clave/idempotency_key con solo un subconjunto de args, hay riesgo de colisión silenciosa. esta verificación es transversal a TODO el plugin — no por modulo aislado. En C30, los 2 fire sites (payout-scheduler.php:683, openpay-webhook-handler.php:349) pasan 3 args; el subscriber en fiscal-annual-close.php:19 solo aceptaba 2 — mismatch por 4 ciclos.

5. **Test AuditCicloN debe cross-check tanto fire-sites como subscribers de cada hook que el modulo tocado escucha.** Los cross-checks previos (C25-C29) solo verificaban la source del auditor — no los fire sites del action. C30 añade: para cada `add_action($hook, $callback, $priority, $accepted_args)` en el modulo auditado, assert que TODOS los `do_action($hook, ...)` en includes/ disparan con args >= accepted_args (text-search literal del `do_action( '$hook', arg1, arg2, ...argN )` y del `add_action( '$hook', ..., $priority, $N )`). Si un fire site pasa menos args que el subscriber declaracomo `accepted_args`, el subscriber recibe `null/0/''` y la idempotency_key (u otra lógica) puede construir mal. Si un fire site pasa MÁS args que `accepted_args`, los extras se descartan silenciosamente — FAC-001 vive aquí.

6. **En compliance fiscal-financiero (DIAN/SAT/Hacienda), `php -l` + test source-based son suficientes para que Suite completa se mantenga verde — pero el real test es trazabilidad fiscal.** C30 coverage via `tags presentes en source` + `idem_key spoofable via source-search-string` documentan el contrato. Test de runtime requeriría mockear `LTMS_Business_Wallet::debit()` (que usa transacciones MySQL reales), `get_user_meta`/`update_user_meta` (stateful), `LTMS_Core_Config::get()` (config), `current_time()` (timezone), `wc_get_order`/`WC_Order`. El ratio costo/beneficio es alto — el source-based test captura el cambio estructural del fix (tag CICLO30 presente, ternario `$payout_id > 0` explicit, accumulated pre-debit removido, branches documentados) sin re-implementar la lógica. La validación end-to-end real (debit GMF verdadero, idempotency_key con payout_id>0) se hace en SG con plugin recargado — ver AGENTS.md "Validación SSH obligatoria antes de commit".

### Leccion 31.1: Cierre de invariante transversal — MPL_mPL dette 5 ciclos, inventario subestimado superficie real, stub bootstrap sirvió en runtime pero también rompió tests que dependen transitivamente

**Contexto:** AUDIT-BUSINESS-CICLO31 — cierre invariante transversal IP (Pendiente 5 ciclos desde C25, item #30 del backlog `CG-28-P2-6 STRONG candidate`). El checkpoint C30 describía el cierre como "2 fixes de 1 linea (deposit:164 + wallet:606)" pero la AUDITORIA C31 reveló que el inventario del checkpoint estaba subestimado: hay **11 ocurrencias activas** de `LTMS_Utils::get_ip()` en includes/, NO 2 — 5 de ellas en rate-limit auth crítico spoofeable (login/register/email-verify/resend-verification brute-force bypass). El usuario eligió "10 restantes (cierre real)" — se excluye `dashboard-logic.php:2491` que ya migró en v2.9.120 con fallback defensivo (válido). Tras migrar los 10 sitios 1:1 (drop-in compatible `(): string`), surgió una **regresión de tests** que la 1a revisión detectó: `PayoutSchedulerTest` fallo con 6 failures porque `LTMS_Payout_Scheduler::create_request` invoca transitivamente `Wallet::execute_transaction` (que ahora llama `get_client_ip_safe`), pero el stub inline de `LTMS_Core_Security` en `tests/bootstrap.php:129-239` no tenía `get_client_ip_safe()` definido — el stub inline previene que el autoloader cargue la clase real. Fix de infraestructura: anadido stub `get_client_ip_safe()` al stub inline (signature idéntica a producción, implementación simplificada). Tras el stub: suite completa 4,433/4,433 verde. 2a revisión OBLIGATORIA APROBADA en primera vuelta (modulos críticos tocados: wallet + auth rate-limit).

**Hallazgo clave 1 — EL INVENTARIO DE UNA INVARIANTE TRANSVERSAL PUEDE SUBESTIMARSE POR 5x:** el checkpoint C30 decia "2 sitios" (deposit + wallet) basado en el test C28 `test_ltms_utils_get_ip_method_still_exists` cuya docstring mencionaba esos 2. Pero un grep `LTMS_Utils::get_ip(` sobre `includes/` revela **11 ocurrencias activas** — el test C28 solo contaba los sitios que el auditor de C28 tuvo en mente al escribir la docstring. Los 9 sitios adicionales (5 rate-limit auth + 2 frontend throttle + 2 auditoria) NO estaban en el radar del test. Regla: cuando una invariante transversal está pendiente de cierre (backlog identificado pero no ejecutado), el inventario se debe CONFIRMAR con grep, no confiar en la enumeración de un test previo que solo menciona los sitios que EL test author tuvo en cuenta al escribir su docstring. La enumeración puede estar subestimada por ordenes de magnitud.

**Hallazgo clave 2 — UNA MIGRACIÓN 1:1 DE METODOS PUEDE ROMPER TESTS POR DEPENDENCIA TRANSITIVA INESPERADA:** la migración `LTMS_Utils::get_ip()` → `LTMS_Core_Security::get_client_ip_safe()` es drop-in compatible (ambos `(): string`) en código de producción. Pero el `tests/bootstrap.php` define un stub inline de `LTMS_Core_Security` (lineas 129-239) ANTES de `vendor/autoload.php` — ese stub previene que el autoloader cargue la clase REAL de `includes/core/class-ltms-security.php`. El stub inline ahí solo define `encrypt`, `decrypt`, `hash`, `verify_hash`, `generate_token`, `generate_referral_code`, `verify_webhook_signature`, `sanitize_email`, `sanitize_document_number`, `sanitize_phone`, `current_user_can`, `derive_key` — NO define `get_client_ip_safe`. Resultado: cuando un test invoca transitivamente una llamada a `get_client_ip_safe()` (via `Wallet::execute_transaction` → `insert ip_address`), el metodo no existe en el stub inline → "Call to undefined method LTMS_Core_Security::get_client_ip_safe()" → propagado como failure en `create_request` (6 tests fallidos). Fix: añadir el stub al stub inline REPITO: cuando una clase de producción tiene un stub inline en `tests/bootstrap.php`, TODO metodo de producción que se invoque en runtime transitivo DEBE estar en el stub inline. La ausencia no se manifiesta en tests que no ejercen la cadena transitiva, pero brota en cualquier test que involucre Wallet/deposit/auth rate-limit. La lección: al añadir un metodo a una clase de producción que tiene stub inline en bootstrap, TAMBIEN añadirlo al stub inline.

**Reglas preventivas:**

1. **Antes de declarar una invariante transversal "cerrada", grep el plugin includes/ entero (no solo los sitios que el test previo enumera).** El test C28 `test_ltms_utils_get_ip_method_still_exists` solo mencionaba 2 sitios (deposit/wallet) porque su author tenía esos en mente al escribir la docstring. El grep real reveló 11 ocurrencias. La subestimación de 2 → 11 (5.5x) es dramatica — significa que el "cierre" hubiera sido cosmético: 2 sitios migrados + 9 spoofeables restantes (5 de ellos rate-limit auth anti-brute-force CRITICO). Regla: `grep -rn 'OLD_PATTERN' includes/` antes de declarar una invariante cerrada. Si el conteo > 1, debe listar TODAS las ocurrencias; el alcance de "cierre" es lo que diga el grep, no lo que diga un test previo.

2. **Cuando una clase de producción tiene stub inline en `tests/bootstrap.php`, los nuevos metodos añadidos a esa clase en producción DEBEN tambien añadirse al stub inline.** El stub inline previene que el autoloader cargue la clase real — el stub se convierte en la implementación visible para PHPUnit. Sin `get_client_ip_safe()` en el stub inline, llamada runtime a ese método en cualquier cadena transitiva (Wallet::execute_transaction → INSERT ip_address en C31, pero podria ser cualquier otra cosa en otro ciclo) rompe con "Call to undefined method". Regla: mantener el stub inline de bootstrap.php sincronizado con los metodos publicos de las clases stubbed inline. Especialmente si la clase es fuente unica de verdad (LTMS_Core_Security, LTMS_Core_Config, LTMS_Core_Logger, etc.).

3. **El rollback de una migración 1:1 drop-in compatible en PHP puede romper tests por dependencia transitiva invisible.** Antes de C31, `LTMS_Utils::get_ip()` era llamado en `wallet.php:606` — Tests pasaban. Tras migrar a `get_client_ip_safe()`, los tests transitivament dependents (PayoutSchedulerTest no toca wallet directamente pero invoca create_request que invoca wallet.get_or_create + execute_transaction) fallaron. La cadena transitiva era invisible al test autor: `create_request` → `Wallet::get_or_create` → carga la clase wallet → `execute_transaction` hace INSERT con `ip_address` → llama `get_client_ip_safe()` → no existe en stub bootstrap → crash. Regla: cuando migras una metodo source-of-truth (IP, fecha, auth, wallet, comisiones), corre la SUITE COMPLETA después — los tests aislados del módulo migrado pueden pasar pero la cadena transitiva puede romper otros modulos cuyos tests cargan el modulo migrado implicitamente.

4. **El "alcance del cierre" no lo decide el autor del ciclo previo, lo decide el grep del ciclo actual.** El checkpoint C30 decia "2 fixes de 1 linea (deposit + wallet)" basado en lo que el autor de C28 habia documentado en el test `test_ltms_utils_get_ip_method_still_exists`. Pero C28 nunca tuvo como objetivo CERRAR la invariante — solo cerro compliance-guardian y dejo el resto como backlog. La enumeración "2" del test C28 era **ilustrativa** (deposit/wallet como ejemplos de lo que quedaba), no exhaustiva. Regla: cuando un checkpoint previo enumera sitios pendientes para un ciclo próximo, el ciclo próximo debe (a) ejecutar grep para enumerar completa, (b) decidir alcance real con el usuario, (c) no asumir que la enumeración del checkpoint anterior era exhaustiva.

5. **Las decisiones de producto sobre alcance son obligatorias cuando el grep diverge de la expectativa del checkpoint.** El checkpoint C30 decia "2 fixes" — el grep revelo 11. Sin preguntar al usuario (4 opciones: 10/8/7/9 restantes), habría migrado solo 2 y declarado "invariante cerrada" — FALSO. La decision era de alcance (cuantos sitios cerrar), NO de tecnica (la migración 1:1 es objetivamente correcta para cualquiera de los 11). Regla: cuando la realidad (grep) diverge del plan (checkpoint) por más de 2x, pausa y pregunta antes de ejecutar. El "cierre real" puede ser 5x más grande de lo que el checkpoint decia — el usuario debe decidir si quiere pagar ese coste ahora o diferirlo.

6. **2a revision OBLIGATORIA en modulos financieros/auth SIEMPRE agrega valor — pero el tipo de valor varia.** C29: NO APROBADO (detectó 1 P1 adicional — fix aplicado → APROBADO). C30: APROBADO en primera vuelta (0 P0/P1 nuevos). C31: APROBADO en primera vuelta (0 P0/P1 nuevos, 3 P2 backlog no-bloqueantes). Las 3 señales son valiosas: la NO-aprobación evita deployar bug latente, las 2 aprobaciones aumentan confianza pre-commit. Ninguna es opcional SI el ciclo toca wallet/comisiones/payouts/auth/KYC/SAGRILAFT/ZapSign/Backblaze/gateways de pago. C31 tocó wallet:606 + auth rate-limit public-auth-handler — entra en la definicion. La 2a revision confirmó que la migración 1:1 drop-in compatible realmente preserva semántica (especialmente: el `ip_address` en INSERT de wallet solo cambia de valor, no de posicion ni tipo; los rate-limit keys `md5($ip)` mantienen su formula — solo cambia la fuente de la `ip`).

### Leccion 32.1: API client hardening (wp_remote_get endpoint externo) + cache-coherence post-write — el metodo flush_* huerfano, invariante write-then-invalidate, patron canonico INTEGRATIONS-AUDIT P1 extendido

**Contexto:** AUDIT-BUSINESS-CICLO32 — modulo `includes/business/class-ltms-business-aveonline-cities.php` (517L pre-fix, 527L post-fix). El modulo sincroniza el catalogo oficial de ciudades de Aveonline desde el endpoint externo `https://app.aveonline.co/assets/resources/public/listadociudades.json` hacia la tabla local `bkr_lt_aveonline_cities` (UNIQUE KEY `uk_nombre`). 5 features: sync() upsert completo, find_by_name() resolucion de texto libre, get_dane_code() resolucion DANE, get_options() dropdown para select2, ajax_search_cities() autocompletar front+back. Cron diario 24h + sync manual admin + sync en activacion/actualizacion del plugin. 2 P1 fixeados en 1 commit `d44cad4a`. 2a revision APROBADA en primera vuelta (modulo hace wp_remote_get a dominio externo + upsert de data externa en DB). 3 P2 backlog nuevos (no introducidos por C32 — preexisting).

**Hallazgo clave 1 — EL METODO `flush_*` HUERFANO:dead code by design** que arregla un bug de cache-coherence POR CONSTRUCCION pero nunca es invocado: `flush_options_cache()` definido en linea 388 hacia `delete_transient('ltms_aveonline_city_options')` (el mismo key que `get_options()` lee en linea 324 y escribe en linea 348). Pero grep `flush_options_cache\(\)` en todo el plugin retornaba 1 match = la definicion misma. Resultado: tras `sync()` exitosa con ciudades renombradas/actualizadas, el dropdown seguia sirviendo data stale por hasta 12h (TTL del transient). El metodo `flush_options_cache()` era **dead code by design**: existe para corregir un bug de cache-coherence pero nadie lo llama — bug latente desde v1.0.0 del modulo. Regla preventiva: auditar cada `set_transient` y cada `delete_transient`/`flush_*_*` — debe haber un 1:1 entre write-ops (INSERT/UPDATE/DELETE en DB) y flushes (cache invalidation) en el mismo flujo. Un `flush_*` definido sin callers es dead-code-by-design — bug de cache-coherence latente. Grep anti-patron: `flush_*|delete_transient.*X` y verificar call sites en el mismo match.

**Hallazgo clave 2 — INVARIANTE WRITE-THEN-INVALIDATE ORDER:** tras detectar el metodo huerfano, el fix no es solo "añadir el call" — importa el ORDEN. `write-op (upsert)` -> `set_transient(last_sync)` -> `flush_cache(options)` -> `log`. Si flush viene ANTES del upsert, un futuro refactor que mueva el upsert a un branch podria flush sin que se haya escrito nada (invalidar cache para nada). Si flush viene antes del set_transient(last_sync), un sync que falle despues del flush podria marcar last_sync como exitoso en un futuro refactor. El orden documentado: write-op -> marcar ok -> invalidar cache derivado -> log. Analogo al orden financiero C30 Leccion 30.1 regla #3 ("operacion atomica -> mutar libro"): aqui es "write-op persistente -> set_transient(last_sync ok) -> flush_cache derivado". Mismo patron en todo modulo que mantiene un transient/option derivado de datos en DB.

**Hallazgo clave 3 — INVARIANTE INTEGRATIONS-AUDIT P1 (sslverify explicito) EXTENDIDA:** la invariante INTEGRATIONS-AUDIT P1 (establecida en C18) exige que toda llamada `wp_remote_get`/`wp_remote_post` a un endpoint externo tenga `'sslverify' = ! ( defined( 'LTMS_DISABLE_SSL_VERIFY' ) && LTMS_DISABLE_SSL_VERIFY )` explicito. C18 la establecio en `class-ltms-api-aveonline.php` (15 sitios con tag "INTEGRATIONS-AUDIT P1 FIX: sslverify explicit (was missing)"). C32 detecto que la invariante NO se habia aplicado a `class-ltms-business-aveonline-cities.php:393` — el modulo NO pertenece a includes/api/ (donde estaba el scope original de C18), pero el endpoint pertenece al mismo dominio externo (app.aveonline.co) y el riesgo es identico (MITM + data injection en DB local sin mas sanitizacion que (string) cast). Regla: la invariante INTEGRATIONS-AUDIT P1 se aplica a TODO modulo que haga wp_remote_get/post a un endpoint externo, NO solo a los de includes/api/. Grep anti-patron: `wp_remote_(get|post)\(` en todo `includes/` y verificar que cada llamada tenga `'sslverify'` explicito. C18 audito includes/api/ pero dejo pendiente includes/business/, includes/frontend/, includes/admin/ — C32 cerro 1 sitio. Pendientes: auditoria completa `wp_remote_*` en includes/business/admin/frontend/ — candidato C33+.

**Hallazgo clave 4 — MODULO SIN INVARIANTE IP NO NECESITA MIGRAR:** el modulo hace fetch externo SIN forward de IP del cliente (no recibe `$_SERVER['REMOTE_ADDR']` ni headers X-Forwarded-For en ninguna parte). La invariante transversal C25/C31 (`get_client_ip_safe`) NO aplica a este modulo. Verificado por grep: `LTMS_Utils::get_ip()|get_client_ip_safe()` en el archivo retorna 0 ocurrencias. Regla: NO todo modulo debe migrar a invariantes transversales — la auditoria debe PRIMERO grep el patron, y solo migrar si hay match. Migrar "por similud" (todos los modulos cambian xxx) sin grep es anti-patron. C32 confirmo explicitamente con test `test_no_ltms_utils_get_ip_in_aveonline_cities` que ni la vieja ni la nueva funcion estan presentes — anti-regresion si un futuro refactor las introduce.

**Hallazgo clave 5 — 2a REVISION EN MODULO NO-CRITICO PERO CON SUPERFICIE EXTERNA:** el modulo no toca wallet/comisiones/payouts/auth/KYC — NO entraba strictamente en AGENTS.md "Revision como ultimo filtro". PERO: hace wp_remote_get a un endpoint externo + ingiere 20000 filas de data externa en DB local con minimas sanitizaciones (`(string) cast + trim` y validacion `is_array`). Si un MITM inyecta JSON malicioso (cosa que AVC-002 previene parcialmente), o si el endpoint cambia formato, los datos se ingieren directamente a la DB. La 2a revision se aplico anyway (decision del agente, no obligatoria por AGENTS.md) y confirmo: 0 P0/P1 nuevos, 3 P2 backlog pre-existing (dead hook listeners ltms_plugin_activated/updated, accounting bug cosmético en `$synced`, race condition en `maybe_sync`). Regla: cuando un modulo hace fetch a endpoint externo + ingiere la data en DB, la 2a revision es recomendable aunque el modulo no toque wallet/auth directo — el riesgo es data corruption indirecto (ciudades malas en DB generan envios malos).

**Reglas preventivas:**

1. **Auditar cada `set_transient` y cada `delete_transient`/`flush_*` — debe haber 1:1 entre write-ops y flushes.** Un `flush_*` definido sin callers es dead-code-by-design — bug de cache-coherence latente que genera data stale silenciosamente. Grep `flush_*\(\)|delete_transient.*X` y verificar call sites. El bug no rompe nada inmediatamente (el cache eventualmente expira por TTL), pero genera UX defectuosa (dropdown no refleja cambios por horas) y solo se manifiesta cuando el data source externo cambia — dificil de detectar en testing normal.

2. **Orden write-then-invalidate:** `write-op persistente` -> `set_transient(last_sync_ok)` -> `flush_cache derivado` -> `log`. Nunca invalidate ANTES del write-op (puede flush sin que se haya escrito nada) ni entre write y set_transient(last_sync) (puede futuro-refactor marcar sync ok tras un flush vacío). Analogo al orden financiero "operacion atomica -> mutar libro" (Leccion 30.1 regla #3): aqui es "write-op -> marcar exito -> invalidar cache derivado". Aplica a todo modulo que mantenga un transient/option derivado de datos en DB.

3. **Invariante INTEGRATIONS-AUDIT P1 (sslverify explicito) se aplica a TODO wp_remote_get/post con endpoint externo, no solo a includes/api/.** C18 audito `includes/api/` y establecio el patron canonico en 15 sitios de api-aveonline.php. Pero la invariante se generaliza a TODO modulo que haga fetch externo: includes/business/ (aveonline-cities.php cerrado C32), includes/admin/ (pendiente), includes/frontend/ (pendiente), includes/core/ (pendiente). Grep `wp_remote_(get|post)\(` en includes/ entero y verificar que cada llamada tenga `'sslverify'` explicito. C33+ candidate: cerrar la invariante en todos los sitios restantes.

4. **NO todo modulo debe migrar a invariantes transversales — primero grep, solo migrar si hay match.** C32 confirmo con test explicito que `class-ltms-business-aveonline-cities.php` NO tiene `LTMS_Utils::get_ip()` ni `get_client_ip_safe()` — el modulo no recibe IP del cliente, solo hace fetch externo. Migrar "por similitud" (todos los modulos cambian xxx) sin grep es anti-patron: anade ruido al diff sin valor. La regla C25/C31 se aplica a modulos que RESUELVEN la IP del cliente, no a los que no la reciben. Auditar cada modulo contra el patron primero, migrar solo si match.

5. **2a revision recomendada (no obligatoria por AGENTS.md, pero aplicada) para modulos con superficie externa + ingesta de data externa en DB.** El modulo no toca wallet/auth pero su wp_remote_get + upsert de 20000 filas foraneas con minimas sanitizaciones es superficie de data corruption indirecto. La 2a revision confirmo 3 P2 backlog preexisting (dead hook listeners, accounting cosmético, race condition) — no detectados por 1a auditoria porque el foco era los 2 fixes nuevos, no los 5 hooks del modulo. Aplicar 2a revision cuando el modulo hace fetch + ingesta de data externa en DB es buena practica aunque AGENTS.md no obligue.

6. **Test source-based cross-check cubre invariantes estructurales (constants, hooks args, guards) NO solo fixes.** El test C32 incluye 4 tests anti-regresion estructural: `SOURCE_URL` constante no cambia (contrato endpoint externo), `MAX_CITIES=20000` + guard `count($data) > MAX_CITIES` en sync() (defensa contra JSON malicioso), `fetch_source` valida `is_wp_error + response_code(==200) + json_last_error + is_array` (defensa en profundidad), `ajax_sync` requiere cap `manage_woocommerce`. Replicar este patron en C33+: los tests anti-regresion de invariantes estructurales son tan valiosos como los tests de los fixes nuevos — previenen que un futuro refactor silenciosamente Elimine defensas previas que el autor no conocia.

### Leccion 33.1: Cierre transversal de invariante INTEGRATIONS-AUDIT P1 sslverify — patron de 5 fases para cierre de invariante transversal, `sslverify => true` hardcodeado es deuda tecnica, fix en scaffolding es defense-in-depth inocuo

**Contexto:** AUDIT-BUSINESS-CICLO33 — cierre transversal de la invariante INTEGRATIONS-AUDIT P1 (establecida C18 sobre 15 sitios de `includes/api/class-ltms-api-aveonline.php`, extendida C32 a 1 sitio en `includes/business/class-ltms-business-aveonline-cities.php` con Leccion 32.1 regla #3). La invariante exige sslverify explicito con override por constante `LTMS_DISABLE_SSL_VERIFY` en TODA llamada `wp_remote_get/post` a endpoint externo HTTPS de TODO `includes/`, no solo `includes/api/`. C33 cierra la invariante en los 32 sitios restantes en 14 archivos (26 MISSING + 6 migrados de `sslverify=>true` hardcodeado) en 1 commit atomico `45ce1623`. 2a revision subagente general APROBADA en primera vuelta (modulo con superficie externa + ingesta de data - Leccion 32.1 regla #5). Decision de usuario (pregunta interactiva) prevalente: incluir los 3 PAC scaffolding de fiscal-annual-close.php (FAC-008/FAC-009 P2 pendientes wire-up — fix inocuo defense-in-depth) + migrar los 6 `sslverify=>true` hardcodeado al patron canonico. Excepcion documentada: `class-ltms-geo-detector.php:54` usa `sslverify=>false` intencional porque el endpoint es `http://ip-api.com` (NO HTTPS, sslverify no aplica para URLs http).

**Hallazgo clave 1 — INVARIANTE TRANSVERSAL COMO DEUDA TECNICA ACUMULADA:** la invariante INTEGRATIONS-AUDIT P1 se estableció en C18 sobre `includes/api/` (15 sitios). Por 15 ciclos (C18→C33), cada nuevo modulo que hacia `wp_remote_*` con endpoint externo HTTPS fuera de `includes/api/` acumulaba deuda tecnica silenciosamente — el modulo funcionaba, los tests pasaban, pero la invariante no se aplicaba. C32 detecto 1 sitio (aveonline-cities) y formalizo la regla #3 (Leccion 32.1: la invariante se aplica a TODO includes/ no solo includes/api/), pero lo dejo como "candidato C33+". C33 demuestra que el cierre de invariantes transversales tiene un patron: (1a fase) inventario con grep `wp_remote_(get|post)\(` en TODO el scope, (2a fase) filtrar comentarios y excluir excepciones documentadas (endpoints http), (3a fase) aplicar el patron canonico a cada sitio + tag, (4a fase) test cardinality-based asserting el total de tags, (5a fase) 2a revision cubriendo que no se altero comportamiento funcional + no se elimino codigo previo. Este patron es replicable a futuros cierres de invariantes transversales pendientes (si las hay).

**Hallazgo clave 2 — `sslverify => true` HARDCODEADO ES DEUDA TECNICA OCULTA:** 6 sitios tenian `'sslverify' => true,` hardcodeado (fx-rate-provider x3, api-deprisa x2, compliance-guardian x1). En la superficie "tienen sslverify explicito" — si el test solo checkea presencia de `'sslverify'`, pasan. PERO la invariante exige OVERRIDE por constante `LTMS_DISABLE_SSL_VERIFY` — si el modulo se prueba en env sin CA bundle (SiteGround staging, dev local sin curl.ca-bundle), los tests PVC/e2e que hagan `wp_remote_*` real fallarian sin override. La migracion a patron canonico respeta el workflow de dev/staging. Regla: `'sslverify' => true` literal hardcodeado (sin `! ( defined(...) && ... )`) es deuda tecnica — se ve correcto en production pero no respeta el override de dev/staging. Test anti-regresion: `assertStringNotContainsString("'sslverify' => true", $source)` cubre este caso. C33Migration:NO dejar `sslverify=>true` hardcodeado en production code aunque el modulo este en scope de `includes/api/` (donde el patron canonico es estandar).

**Hallazgo clave 3 — FIX EN CODIGO MUERTO (SCAFFOLDING) ES DEFENSE-IN-DEPTH INOCUO:** los 3 sitios en `class-ltms-fiscal-annual-close.php` (PAC Facturama, SW Sapien, Edicom) son scaffolding marcado P2 (FAC-008/FAC-009 — pendiente wire-up de `ltms_cfdi_request` que nunca se dispara en production). La pregunta interactiva al usuario ofrecio 3 opciones: (a) aplicar fix a los 32 sitios incluido 3 PAC, (b) excluir 3 PAC (29 fixes), (c) excluir PAC + traffic-booster (24 fixes). El usuario eligio (a) — argumento: sslverify canonico en PAC protege si se wire-up en el futuro (no dana, defense-in-depth); si nunca se wire-up, el fix es inocuo. El override de constante `LTMS_DISABLE_SSL_VERIFY` respeta dev/staging. Regla: aplicar invariantes transversales a TODO el scope (incluido scaffolding) es preferible a excluir por "codigo muerto" — defense-in-depth no dana, y si el scaffolding se wire-up futuro, ya esta protegido. La decision sigue siendo del usuario cuando hay ambiguedad (AGENTS.md "Decisiones de producto"), pero el default razonal es "aplicar Always".

**Hallazgo clave 4 — TEST PARAMETRICO DATA-DRIVEN PARA CIERRE TRANSVERSAL:** el test C33 usa `@dataProvider provide_expected_c33_tags` con 32 entries (file+tag), iterando cada caso como test individual. Misma estructura para `provide_touched_files` (14 entries) y `provide_hardcoded_migration_files` (3 entries). Ventaja: si el proveer de data falla (e.g. un archivo no existe, un tag no se encontro), la salida PHPUnit especifica cual caso fallo — mas legible que un solo test monolito. Ademas, `test_total_c33_tags_count_is_32` cubre cardinalidad — anti-anadir/quitar silencioso. Sin embargo, la limitacion es que el dataProvider es hardcoded al estado actual de C33 — un nuevo `wp_remote_*` HTTPS sin sslverify futuro no lo detectaria. El subagente revisor sugirio un test parametrico por reflexion/tokenizacion del source que enumere `wp_remote_*` con URL HTTPS via parsing — queda como backlog C34+ para cuando la invariante pueda romperse por adicion futura. Por ahora, el test C33 cubre el estado de cierre del ciclo (snapshot-based). Regla: en cierre de invariante transversal, test dataProvider hardcoded + test cardinality es sufficiente para anti-regresion actual; el test de regex dinámico para detectar futuras adiciones sin sslverify es backlog si la invariante tiene riesgo de romperse por nuevas features.

**Hallazgo clave 5 — GOTCHA EDIT CON BLOQUE `$payload = [...]` ADYACENTE A `wp_remote_post`:** en `vendor-invoicing-generator.php`, el fix `alegra_get_or_create_contact` metodo hizo un edit que sustituyo `// Crear contacto nuevo.\n$response = wp_remote_post( ... )` (con sslverify added), pero el `oldString` seleccionado no incluia explicitamente el bloque `$name_parts = explode(...); $payload = [...]` que estaba ENTRE el comentario y la llamada. El edit se aplico "correctamente" (sintaxis PHP ok, `php -l` paso), pero el bloque `$payload` se PERDIO — el test PHPUnit individual NO capture porque el test es source-based y no runtime-avigile los datos, pero el C33 2a revision subagente detecto el problema en cross-check. Fix rapido: re-anadir el bloque `$payload` con replaceAll=false en el sitio correcto. Regla preventiva: en `edit` tool, cuando el bloque a cambiar tiene codigo adyacente (variables, payloads) entre el comentario y la llamada, INCLUIR el bloque entero en `oldString` y `newString` o hacer dos edits separados (uno para el comentario, otro para la llamada) — evitar "selecciones quirurgicas" que omitan codigo intermedio. Aplica a cualquier wp_remote_get/post con payload previo. La 2a revision sigue siendo la red de seguridad mas confiable para estos casos.

**Hallazgo clave 6 — EXCEPCION DOCUMENTADA COMO INVARIANTE NO-APLICA:** `class-ltms-geo-detector.php:54` usa `sslverify => false` intencional. En la superficie, parece que viola la invariante (un `wp_remote_get` sin patron canonico). PERO el endpoint es `http://ip-api.com` (NO HTTPS). Para URLs `http://`, sslverify no aplica — el stream context creator lo ignora si se pasa, y `false` es lo correcto para no confundir. El modulo antibot sí verifica explicitamente via test `test_geo_detector_keeps_sslverify_false_for_http_endpoint` que el `sslverify=>false` se preserva + que NO hay tag C33 + que NO se introdujo `LTMS_DISABLE_SSL_VERIFY` en el archivo. Regla: en cierre de invariante transversal, las excepciones documentadas (endpoints http, mocks, tests) se preservan con test explicito anti-regresion — la invariante se DOCUMENTA como no-aplica, no se "silenciosamente ignora". Mismo patron que Leccion 32.1 regla #4 (modulo sin IP no migra a get_client_ip_safe, documentado con test `test_no_ltms_utils_get_ip_in_aveonline_cities`).

**Hallazgo clave 7 — CREENCIA EXTENDIDA: 2a REVISION APLICADA EN COMMIT TRANSVERSAL (NO MODULO ESPECIFICO):** AGENTS.md "Revisión como ultimo filtro" exige 2a revision en modulos CRITICOS (wallet, comisiones, payouts, KYC/SAGRILAFT, ZapSign, Backblaze, gateways de pago). Leccion 32.1 regla #5 extiende a "modulos con superficie externa + ingesta de data externa en DB". C33 es un COMMIT TRANSVERSAL — toca 14 archivos de 4 submodulos distintos, algunos con superficie externa, otros no. La 2a revision se aplico anyway y confirmo 0 P0/P1 nuevos + 1 P2 typo (corregido pre-commit). Regla: en commit transversal (>5 archivos tocados), 2a revision RECOMENDADA aunque no toque modulos criticos — el riesgo es distinto: romper algo en un archivo individual, no una feature critica. La 2a revision cubre (a) patron canonico byte-a-byte uniforme, (b) no se elimino codigo previo (gotcha del bloque `$payload`), (c) no se altero comportamiento funcional (timeouts, headers, body, hooks). Patron replicable a futuros cierres transversales (C34+ si los hay): 2a revision siempre recomendada en commit atomico multi-archivo.

**Reglas preventivas:**

1. **Cierre de invariante transversal = patron de 5 fases:** (1a) inventario grep `wp_remote_(get|post)\(` en TODO el scope, (2a) filtrar comentarios y excluir excepciones documentadas (endpoints http, mocks, tests), (3a) aplicar patron canonico a cada sitio + tag `CICLO{N}-P1-SSL-{ID} FIX`, (4a) test cardinality-based asserting el total de tags exactos, (5a) 2a revision cubriendo patron byte-a-byte + no se elimino codigo previo + no se altero comportamiento. Replicable a futuros cierres de invariantes transversales pendientes (si las hay — pendiente incluir `LTMS_Core_Security::get_client_ip_safe` en cualquier nueva llamada a IP, etc).

2. **`'sslverify' => true,` hardcodeado es deuda tecnica:** en la superficie "tiene sslverify explicito" pero no respeta override por `LTMS_DISABLE_SSL_VERIFY` para dev/staging sin CA bundle. Migrar a patron canonico `! ( defined( 'LTMS_DISABLE_SSL_VERIFY' ) && LTMS_DISABLE_SSL_VERIFY )`. Test anti-regresion: `assertStringNotContainsString("'sslverify' => true", $source)` en modulos migrados. No dejar `sslverify=>true` hardcodeado en production code aunque este en scope `includes/api/`.

3. **Fix en codigo muerto (scaffolding) es defense-in-depth inocuo:** aplicar invariantes transversales a scaffolding (PAC adapters, features deshabilitadas via flag, hooks sin fire-sites) protege si se wire-up en el futuro. No dana. Default razonable: aplicar Always a TODO el scope. Si hay ambiguedad (codigo muerto con riesgo real de romperse), preguntar al usuario (AGENTS.md "Decisiones de producto") — pero el default es aplicar.

4. **Test cardinality-based en cierre transversal:** el test `test_total_c33_tags_count_is_32` asegura que si alguien anade/quita un fix, el test fallo con mensaje claro. Es barato (substr_count en source) y previene regressiones silenciosas. Combinar con `@dataProvider` por archivo (test individual por fix) para actionable failure messages. El test de regex dinámico para detectar futuras adiciones sin sslverify es backlog C34+ — por ahora el snapshot-based cubre estado actual.

5. **Excepcion documentada es invariante no-aplica, no silenciosamente ignorada:** `class-ltms-geo-detector.php:54` con `sslverify=>false` por endpoint http se preserva con test explicito `test_geo_detector_keeps_sslverify_false_for_http_endpoint` cubriendo: `sslverify=>false` presente, `LTMS_DISABLE_SSL_VERIFY` ausente del archivo, tag C33 ausente. Regla: en cierre de invariante transversal, las excepciones se documentan en test — la invariante se "cierra explicitamente no-aplica", no se salta silenciosamente. Mismo patron que Leccion 32.1 regla #4.

6. **Edit con codigo adyacente: incluir bloque entero o editar en partes:** en `vendor-invoicing-generator.php`, el fix de `alegra_get_or_create_contact` perdio un bloque `$payload = [...]` porque el `oldString` seleccionado no incluia el bloque intermedio entre comentario y llamada. El edit se aplico (sintaxis ok) pero el runtime hubiera fallado. La 2a revision subagente detecto el problema via cross-check del source. Fix rapido: re-anadir con replaceAll=false. Regla preventiva: en `edit` tool, cuando el bloque a cambiar tiene codigo adyacente (variables, payloads, asignaciones) entre el comentario y la llamada a wp_remote_*, INCLUIR el bloque entero en `oldString` y `newString` o hacer dos edits separados (uno para el comentario, otro para la llamada) — evitar selecciones quirurgicas que omitan codigo intermedio. La 2a revision sigue siendo la red de seguridad mas confiable.

7. **2a revision en commit transversal (>5 archivos tocados) RECOMENDADA:** C33 toca 14 archivos en 1 commit atomico. La 2a revision confirmo 0 P0/P1 + 1 P2 typo (corregido pre-commit). El riesgo en commit transversal es distinto a modulo single-file: romper algo en un archivo individual, no toda una feature critica. La 2a revision cubre (a) patron canonico byte-a-byte uniforme entre archivos, (b) no se elimino codigo previo (el gotcha del bloque `$payload`), (c) no se altero comportamiento funcional (timeouts, headers, body, hooks intactos), (d) excepciones documentadas preservadas. Patron aplicable a futuros cierres transversales (C34+).

---

### Leccion 34.1: Deep audit de modulo compliance legal con fixes aplicados por sesion previa + deteccion de hook fire-site muerto + test orfano por cambio de contrato de retorno

**Contexto:** AUDIT-BUSINESS-CICLO34 - deep audit de `includes/business/class-ltms-tourism-compliance-ext.php` (NT-3 a NT-6 FONTUR Colombia Ley 2068/2020) + `class-ltms-business-tourism-compliance.php` (base RNT/SECTUR). Pendiente desde C30 Prioridad 2 (deuda 4 ciclos C30→C34). C33 ya habia cerrado sslverify en este modulo (linea 98) pero NO audito timeouts, response_code, sanitizacion response JSON, ni wire-up de hooks. Al reanudar C34 desde checkpoint C33 detuvo un trabajo en progreso de otra sesion en working tree (TC-001..TC-004 sin commit), mas un hallazgo nuevo H7 detectado y fixeado en esta sesion.

**Hallazgo clave 1 - SESION PREVIA SIN CHECKPOINT + TRABAJO EN WORKING TREE:** Al reanudar C34 el `git status` revelo 2 archivos tracked Modified (no listados en el checkpoint C33 anterior como "ARCHIVOS TOCADOS"). El checkpoint decia "git status --short solo untracked (.opencode/, docs/temp/, docs/tutorial-codewiki-ltms.md, opencode.json)". Discrepancia detectada → investigar `git diff HEAD` ANTES de acumular. Resultado: otra sesion arranco C34 con 4 fixes (TC-001..TC-004) sin commit ni checkpoint. Esta sesion valido los 4 fixes previos, detecto H7 nuevo (TC-005), aplico fix + test + 2a revision + suite + commits. Leccion: el flujo `/continuar` DEBE (a) comparar `git status` actual vs el `git status` declarado en el checkpoint; (b) investigar cualquier M/A/D tracked no listado via `git diff HEAD`; (c) no descartar el trabajo previo (validarlo reusarlo) salvo que este corrupto. El baseline C33 (tags Sum=32, git log 25d542bb HEAD) sigue intacto - el trabajo previo es C34, no reset.

**Hallazgo clave 2 - HOOK FIRE-SITE MUERTO COMO P1 DE COMPLIANCE LEGAL:** TC-004 (H2) detecto que el listener `auto_verify_rnt_fontur` estaba registrado via `add_action('ltms_save_rnt', ..., 10, 2)` en `class-ltms-tourism-compliance-ext.php:20` pero `do_action('ltms_save_rnt')` JAMAS se llamaba en todo el codebase (grep 0 ocurrencias). Resultado: la verificacion automatica NT-3 RNT contra FONTUR era **dead-code-by-design** — análogo a AVC-DEADHOOKS C32 (#36 backlog). Diferencia vs AVC-DEADHOOKS: ahi el hook se registro para "el kernel lo dispara algun dia" (intencion ambigua), aqui el hook se registro para disparar desde `ajax_save_rnt` pero el fire-site nunca se agrego — bug desde la implementacion del listener (3 sin commit en cualquier baseline anterior). Regla: cuando veas `add_action('ltms_*', ...)` custom (no WC core hook), grep `do_action('ltms_*')` inmediatamente. Si 0 matches → el listener es dead-code y su feature NO funciona (independientemente de lo que diga el docblock).

**Hallazgo clave 3 - CAMBIO DE CONTRATO DE RETORNO DESTRUYE TEST ORFANO (Leccion 32.1 regla reafirmada con twist):** TC-005 cambio `save_rnt(): bool` a `save_rnt(): int|false` para distinguir 0-filas-idempotente (re-guardado identico) de false-error (BD). El test existente `TourismComplianceTest::test_save_rnt_return_type_is_bool` (linea 360) seguia sintacticamente valido (la assertion `assertSame('bool', ...)` es codigo PHP valido) pero FALLO en runtime: `Expected 'bool', Actual 'int|false'`. Sin la suite completa (corrida en esta sesion), el test huérfano hubiera esperado silenciosamente hasta un commit futuro no relacionado para romper la suite — exactamente el patron Leccion 32.1 regla #119. Twist C34: el test se actualizo en el MISMO commit del fix, no en commit separado. Pero la suite completa revelo el test_orfano; el test individual nuevo (`AuditCiclo34TourismComplianceTest`) PASABA porque no validaba el tipo de retorno viejo - solo valide la suite completa la pestania del contrato. Leccion: cuando un fix cambia un TIPO DE RETORNO UNION (PHP 8.0+ `int|false`, `string|null`, `array|null`), grep tests por `ReflectionMethod(..., 'metodo')` + `getReturnType` para localizar snapshots de contrato orfano. La suite completa sigue siendo la unica red que detecta todo.

**Hallazgo clave 4 - DEEP AUDIT EXTENDE MAS ALLA DE LO QUE TOCO C33:** C33 cerro `sslverify` en 32 sitios transversales, deep-touch superficial en `tourism-compliance-ext:98` (1 sitio). C34 volvio al mismo modulo para deep audit completo NO de sslverify (ya cerrado) sino de timeouts/response_code/sanitizacion/fire-site de hooks. Hallazgos C34: H1 (response_code faltante), H3 (strpos fragil), H4 (timeout 10 corto), H5 (wp_mail sin check retorno), H2 (hook fire-site ausente), H7 (return type idempotente). Conclusion: el cierre transversal C33 NO es deep audit - es barrido superficial. Cada modulo tocado transversalmente puede tener deuda tecnica profunda adicional. Patron: tras un cierre transversal P1, cada modulo tocado en ese cierre es CANDIDATO para deep audit en C+1.

**Hallazgo clave 5 - HALLAZGO FUERA DE ALCANCE DETECTADO POR 2A REVISION:** La 2a revision subagente general (obligatoria por modulo compliance legal) detecto H9 (regex `^\d{1,5}$` en `auto_verify_rnt_fontur:57` no acepta el formato alfanumerico `CO-BOG-12345678` sugerido en el placeholder UI en `class-ltms-business-tourism-compliance.php:356`). Pre-existente en HEAD (no introducido por C34), fuera de alcance - documentado como backlog P2 #40 para C35. Requiere decision de producto (fixar regex o cambiar placeholder). Leccion: la 2a revision ademas de validar el diff, debe escanear el CONTEXT del modulo para hallazgos pre-existentes candidatos a backlog. No forzar todos los hallazgos relacionados dentro del ciclo actual — algunos son de другой alcance y requieren decision humana.

**Hallazgo clave 6 - DECISION DE NEGOCIO PAUSADA PARA VERIFICACION FUNCIONAL EN SG:** H8 (wp_remote_post a endpoint GET-form FONTUR — posible 405 Method Not Allowed) NO se fixeo en C34. Decision de usuario via `question` tool: "Documentar como backlog P2 — verificacion funcional en SG requerida". El test C34 incluye `test_h8_backlog_documented_post_method_preserved` como snapshot anti-regresion: si alguien "arregla" el metodo HTTP sin pasar por decision de negocio (sin curl real al portal en SG), el test FALLA y fuerza la pausa. Leccion: los hallazgos que requieren decision funcional + validacion externa (curl real a un portal publico) deben (a) pausarse con question al usuario, (b) documentarse en LECCIONES_APRENDIDAS.md con razon explicita de backlog, (c) snapshot anti-regresion en test source-based para impedir "fixes silenciosos" futuros. La regla "no ser too proactivo" aplica doble a cambios que requieren observacion externa (HTTP real, browser, vendor API).

**Hallazgo clave 7 - TEST SNAPSHOT ANTI-REGRESION COMO BARRERA DE DECISION:** El patron de `test_h8_backlog_documented_*_preserved` (snapshot del estado actual hasta que decision de negocio lo cambie formalmente) es una extension del patron anti-regresion usual. Anti-regresion normal: "el codigo bueno sigue bueno". Anti-regresion backlog: "el codigo malo (declarado backlog) sigue malo hasta que decision explicita lo cambie — impedir fixes silenciosos que atajan la discusion". Aplicable a: cambios de metodo HTTP, cambios de tasas de comision, cambios de umbrales regulatorios, decision de vendor. El test se elimina en el commit que cierra el backlog (despues de la decision tomada).

**Reglas preventivas para la IA (C34):**

1. **Comparar `git status` del checkpoint vs working tree actual ANTES de acumular fixes:** el flujo `/continuar` debe (a) leer `git status --short` real; (b) comparar contra el `git status` declarado en la seccion "ARCHIVOS TOCADOS EN ESTA SESION" del checkpoint; (c) si hay M/A/D tracked no listados, investigar via `git diff HEAD` antes de continuar. Puede ser trabajo previo sin checkpoint de otra sesion (como C34 detecto) o reset de working tree (perdida de fixes).

2. **Hook fire-site check inmediato al ver `add_action('ltms_*', ...)`:** cualquier hook custom `ltms_*` (no WC core) registrado con `add_action` debe tener un grep `do_action('ltms_*')` con >= 1 match. Si 0 matches → dead hook, la feature que supuestamente escucha NO funciona (independientemente del docblock del listener). Documentar como hallazgo P1 (no P2 — la feature esta rota funcionalmente).

3. **Cambio de contrato de retorno UNION type requiere grep tests ReflectionMethod + getReturnType:** al cambiar un metodo de `bool` a `int|false` (o `string` a `string|null`, etc), buscar tests que hagan `ReflectionMethod($class, $method)` + `assertSame('bool', ...)` / `assertSame('string', ...)`. Esos tests snapshot el contrato viejo y rompen en runtime sin que el test individual del nuevo fix lo detecte. Suite completa es obligatoria post-cambio de tipo.

4. **Deep audit post-cierre-transversal:** cada modulo tocado superficialmente por un cierre transversal P1 (C18, C33) es CANDIDATO a deep audit en C+1. El cierre transversal es barrido horizontal (1 invariante enforcement en N archivos); el deep audit es barrido vertical (todas las invariantes del modulo: timeouts, response codes, sanitizacion, fire-sites, return types, dead code). C33 + C34 es el patron: cierre transversal sslverify → deep audit tourism-compliance-ext.

5. **2a revision debe escanear context del modulo, no solo diff:** ademas de validar el diff, la 2a revision subagente revisa el contexto alrededor del diff para hallazgos pre-existentes candidatos a backlog (compatibilidad de formatos UI/backend, regex vs placeholders, headers faltantes en otras llamadas). No forzar todos los hallazgos en el ciclo actual — los pre-existentes van a backlog con ID y rationale, se tratan en ciclo aparte (con decision de producto si aplica).

6. **Hallazgos que requieren verificacion funcional externa → pausa + decision de usuario + snapshot anti-regresion:** los hallazgos que dependen de un observable externo (HTTP real a un portal, vendor API response, browser behavior) NO se fixean por intuicion. Se pausan con `question` tool, se documentan como backlog con rationale explicita, y se snapshot el estado actual en un test source-based anti-regresion. El test se elimina cuando el backlog se cierra formalmente (post-decision).

7. **Leccion 33.1 regla #6 (edit con bloque adyacente) VALIDADA y NO regredida:** en C34 se aplicaron 5 fixes con edit tool (TC-001..TC-005), todos con bloques adyacentes (variables locales `$code`, `$body`, `$rnt_number`, payloads, branches `if ($record)`), sin perdida de codigo intermedio. La 2a revision confirmo explicitamente "ningun bloque adyacente perdido" en ambos archivos. La regla 33.1 #6 (incluir bloque entero en oldString/newString o hacer 2 edits separados) se respetó y el patron funciona. Leccion C34: la regla anterior es reproducible - aplicarla siempre con bloques adyacentes.

---

### Leccion 34.2: Re-auditoria sobre modulo ya auditado — coverage gaps del ciclo previo aparecen en JS/templates, no solo en handlers PHP

**Ciclo:** Sub-ciclo RE-AUDIT-AUTH (re-auditoria auth/login/registro vendedores + Google OAuth, posterior al ciclo AUTH-AUDIT original AUTH-01..AUTH-10 cubiertos por `AuthAuditFixTest.php`).

**Contexto:** El ciclo AUTH-AUDIT original audito el handler PHP `class-ltms-public-auth-handler.php` (1,735L) + `class-ltms-google-oauth.php` (571L) a fondo e identifico 10 hallazgos (AUTH-01..AUTH-10). La re-auditoria re-aplico el loop de AGENTS.md sobre el MISMO modulo. Resultado: 4 hallazgos nuevos (2 P1 + 2 P2), 0 P0 (los P0 ya cerrados por el ciclo previo son no-regresion — verificacion explicita en el test nuevo `AuthReAuditFixTest.php` con 9 cross-checks no-regresion).

**Hallazgos claves de la re-auditoria:**

1. **Coverage gap del ciclo previo: JS y templates (no solo handlers PHP).** El ciclo AUTH-AUDIT original se concentro en el handler PHP (`class-ltms-public-auth-handler.php`) y el Google OAuth (`class-ltms-google-oauth.php`). Los templates `form-login.php` y `form-register.php`, y el JS `ltms-login-register.js` quedaron fuera del escrutinio. La re-auditoria detecto 3 de 4 hallazgos nuevos en estos archivos: (a) AUTH-RA1 — `form-register.php` no cerraba `</form>` (P1, bug HTML); (b) AUTH-RA2 — `form-login.php` tenia `wp_nonce_field` codigo muerto (P2); (c) AUTH-RA4 — `ltms-login-register.js` ignoraba `data.data.redirect` en branch error (P1, rompia UX del fix AUTH-01 previo). Solo AUTH-RA3 (clave `'number'` duplicada en PHP) estuvo en el handler. **Regla: una auditoria de "auth" que solo mire handlers PHP NO es completa. Los templates y el JS son parte del flujo E2E y tienen bugs propios que el backend no puede compensar.**

2. **Cambio de contrato UX entre backend y JS: el fix AUTH-01 separa efecto de intención.** El fix AUTH-01 (ciclo previo, P0) anade al response de error el campo `redirect` con URL de reenvio de verificacion. El handler PHP construye esa URL y la manda. PERO el JS del login (`ltms-login-register.js:141-143` original) solo usaba `data.data.message`, NO `data.data.redirect`. Resultado: el backend disenia un mecanismo UX (redirect automatico al form de reenvio) que el JS no implementa. La intencion del fix AUTH-01 ("vendor con email no verificado es llevado al mini-form de reenvio") era literalmente falsa en runtime — el vendor veia el mensaje pero NO era redirigido. La cobertura del ciclo AUTH previo verifico que el backend retornaba `redirect` (test_01b), pero NO verifico que el JS lo siguiera en branch error. **Regla: cuando un fix de un handler anade un campo al response (redirect, errors estructurado, flag booleano), el auditor DEBE verificar que el JS consumer que se comunica con ese handler haga algo con el campo nuevo. Un response con redirect que el JS ignora es un fix incompleto.**

3. **Bug HTML de ausencia de tag de cierre es detectable solo inspeccionando el template completo, no grep aislado.** `form-register.php` abre `<form id="ltms-register-form">` en linha 53 pero nunca lo cierra con `</form>`. La ausencia de un tag no se detecta con `grep '</form>'` (que retorna 0 matches y podria parecer "OK" si no se busca la presencia obligatoria). Se detecta solo al leer el template completo y notar el desbalance: el `</div><!-- .ltms-register-card -->` cierra el card con el form aun abierto, dejando el `.ltms-auth-footer` dentro del form. El bug produce HTML semanticamente invalido, `form.reset()` del JS resetearia los links del footer, y el wizard step nav quedaba semantico dentro del form. **Regla: al auditar un template PHP con wizard o form multi-step, contar `<form` vs `</form>` — deben ser iguales. Similar a contar `<?php` vs `endif endwhile` en templates WP.**

4. **Codigo muerto en tag de nonce es codigo muerto silencioso — no se reporta como error pero contamina el DOM.** `form-login.php:124` tenia `wp_nonce_field('ltms_vendor_login', 'ltms_login_nonce')` — un input hidden muerto. El AJAX handler verifica `check_ajax_referer('ltms_auth_nonce', 'nonce')` con un nonce que viaja via `wp_localize_script('ltmsAuth')`. El `wp_nonce_field` del form nunca se usaba. M-2 elimino el `wp_nonce_field` en `form-register.php` pero no en `form-login.php` (inconsistencia entre templates hermanos). El bug no causa fallo funcional pero: (a) ensucia el DOM, (b) confunde a un lector que asume que el form usa el nonce ahi presente, (c) dispara warnings en analyzers estaticos. **Regla: cuando veas `wp_nonce_field` en un template, trazar explicitamente DONDE se verifica (`check_ajax_referer`) con el MISMO action y MISMO field-name. Si el JS envia otro nonce via `wp_localize_script`, el `wp_nonce_field` es muerto. Aplicar el mismo principio a `wp_nonce_field`, `wp_referer_field`, etc.**

5. **Claves duplicadas en arrays PHP son codigo confuso, no bug funcional (pero el analyzer lo flagga).** `array('number' => 1, 'fields' => 'ID', 'number' => 1)` — PHP silenciosamente usa el ultimo valor del dup key. Aqui ambos eran `1`, sin efecto funcional. Pero un analyzer estatico (PHPStan level 6+, Psalm) lo flagga como error, y un futuro cambio "cambiar el primer `number` a 5 para mejorar performance" tendria efecto nulo (el ultimo `number => 1` gana). **Regla: aunque no haya bug funcional, las claves duplicadas son ruido que se debe eliminar — son indicadores de descuido en una revision.**

6. **2 falsos positivos detectados y rechazados en re-auditoria (no todo hallazgo aparente es real).** La re-auditoria produzco 6 hallazgos aparentes; 4 fueron reales, 2 fueron falsos:
   - Falso 1 (H-N3): presunto falto de `LTMS_Referral_Tree::register_node` en registro normal → FALSO: el listener `LTMS_Affiliates::on_vendor_registered` (priority 10, 2 args) en `class-ltms-affiliates.php:41` invoca `register_node()` via `do_action('ltms_vendor_registered', $user_id, $referral_code)` disparado en auth-handler:753. El hook fire-site SI existe.
   - Falso 2 (H-N4): presunto bypass de `profile_incomplete` tras auto-login en `handle_email_verification` → FALSO: el dashboard `class-ltms-dashboard-logic.php:242-248` hace la guarda leyendo `ltms_profile_incomplete` y redirigiendo a `?complete_profile=1` si falta. La seguridad no esta SO en el endpoint de login — esta distribuida entre endpoint de login + dashboard render.
   **Regla: ante un hallazgo aparente, antes de fixear, GREP el fire-site o el handler complemento. La seguridad en WP se compone frecuentemente de 2 guarda en 2 puntos (login endpoint + dashboard/template) — solo viendo uno se concluye erradamente "falta proteccion". Leccion analog a C34.1 regla #2 (hook fire-site check inmediato).**

**Reglas preventivas derivadas:**

1. **Cobertura de auditoria auth = handlers PHP + templates + JS.** Auditar solo handlers PHP deja gaps en templates y JS. Un loop de auditoria auth completo debe leer los 3 layers y trazar el flujo E2E: template (form HTML) → JS (FormData + fetch) → handler (check_ajax_referer + sanitizacion + persistencia) → response (message + redirect + errors) → JS consumer del response (display + redirect + clear). Cada transicion es un sitio donde puede haber bug.

2. **Response field nuevo en handler exige grep en JS consumer.** Cuando un fix anade un campo al response (`redirect`, `errors` estructurado, `email_verification_required` flag, etc.), grep `data.data.{campo}` en el JS consumer. Si 0 matches → el JS ignora el campo y el fix UX esta incompleto. Aplicar tambien a cambios de nombre de campo: si se renombra `data.redirect` a `data.redirect_url`, grep `data.data.redirect` en JS y actualizar a `data.data.redirect_url` en el MISMO commit.

3. **Tag de cierre `</form>` es la primera verificacion al leer un template con wizard.** Contar `<form` vs `</form>` — deben ser iguales. Mismo para `<div` vs `</div>` (aunque PHP lo hace menos critico por templates parciales). Es un check de 2 segundos que salva bug HTML de horas.

4. **`wp_nonce_field` en template DEBE rastrearse al `check_ajax_referer` que lo consume.** Si el JS usa otro nonce (via `wp_localize_script`), el `wp_nonce_field` es muerto. Aplicar a par de templates hermanos (login/register, my-account/checkout, etc.) — si uno lo elimino, el otro probablemente tambien deba.

5. **2 guarda en 2 puntos es un patron comun WP — no concluir "falta proteccion" viendo solo 1.** Antes de declarar "flujo X bypassable", verificar si hay un segundo punto de guarda (dashboard render, template redirect, hook de redirect, capability check endpoint-siguiente). Ejemplo: `handle_email_verification` auto-loguea sin check `profile_incomplete`, PERO el dashboard lo rechaza y redirige a `?complete_profile=1`. La proteccion es distribuida, no centralizada.

6. **Claves duplicadas en array PHP — eliminar aunque no haya bug funcional.** Son ruido, confunden a futuros edits, y analyzers estaticos las flagean. La eliminacion es trivial y reduce surface de confusion.

7. **Test de re-auditoria debe incluir cross-checks NO-REGRESION contra el ciclo previo.** El test `AuthReAuditFixTest.php` incluye 9 tests cross-check no-regresion contra el ciclo AUTH-AUDIT previo (AUTH-01, AUTH-02, AUTH-04, AUTH-05, AUTH-06, AUTH-08, AUTH-09, AUTH-10) + cross-checks contra invariantes transversales (C33 sslverify en Google OAuth, C31 get_client_ip_safe en auth handler, wp_localize_script ltms_auth_nonce en los 2 sitios canonicos). La re-auditoria NO puede romper los fixes previos — el test lo garantiza. Patron analogo al test C34 que incluye cross-checks C25/C28/C29/C30/C32/C33.

---

### Leccion 35.1: QA funcional contra server real detecta bugs UX que el source-based misses — y `typeof` strings vs objects en responses heterogeneos

**Ciclo:** Sub-ciclo UX-AUDIT-REGISTER (auditoria UX/QA login+registro vendedores, posterior a RE-AUDIT-AUTH `3c97c2fa`).

**Contexto:** El sub-ciclo RE-AUDIT-AUTH previo cerro 4 hallazgos con tests source-based estructurales (AuthReAuditFixTest, 21 tests). Esta vez se aplico el mismo loop de AGENTS.md PERO con un paso adicional: **QA funcional contra server real** (8 tests AJAX via Invoke-WebRequest con UA real a /login-vendedor/ y /registro-vendedor/, capturando nonces y POST bodies). El WAF de SiteGround bloquea Puppeteer Headless (403), pero Invoke-WebRequest con UA de Chrome desktop funciona. La QA funcional detecto 2 P1 (UX-001, UX-002) que la auditoria source-based sola NO hubiera encontrado — la fuente de los bugs solo se ve cuando el sistema responde en runtime con datos reales.

**Hallazgos claves de la auditoria UX/QA:**

1. **`success:true` puede venir sin `email_verification_required` — y el JS debe distinguir el caso.** El backend `class-ltms-public-auth-handler.php:574` retorna `wp_send_json_success(['message' => 'Ya existe una cuenta...', 'redirect' => ''])` cuando alguien intenta registrarse con un email ya existente (patron anti-enumeration — no confirma al atacante si el email esta registrado). El JS previo tenia 4 branches en el register: (a) `isCompleteProfile` (Google OAuth), (b) `data.data.email_verification_required` (registro real), (c) `data.data.redirect` (auto-login), (d) `else` generico. NONE de los 4 matcheaba el caso de email ya existente (sin `email_verification_required`, `redirect=""` vacio). Caida al `else` → "¡Cuenta creada! Revisa tu email..." INCOHERENTE con el message del server "Ya existe una cuenta con este email. Te enviamos un correo con el enlace de inicio de sesion". **Regla: cuando un backend usa `success:true` para varios casos distintos (registro real, email ya existe, auto-login, Google OAuth callback), el JS DEBE distinguir TODOS los casos por flags explicitos (`email_verification_required`, `redirect` truthy, `is_complete_profile`, etc.).Nunca asumir que el `else` es "registro exitoso" — el `else` es "caso no cubierto explicitamente", y cualquier futuro fix del backend que anada un nuevo caso caera ahi silenciosamente.**

2. **`wp_send_json_error($string, $code)` retorna `data.data` como STRING, no como objeto con `.message`.** El rate limit register (`class-ltms-public-auth-handler.php:462`) viaja como `wp_send_json_error(__('Demasiados registros...', 'ltms'), 429)`. PHP serializa eso como `{"success":false,"data":"Demasiados registros..."}  ` (data.data es el string). Pero otros handlers del mismo archivoviajan como `wp_send_json_error(['message' => '...', 'errors' => [...]])` (data.data es objeto). El JS tenia `data.data.message` hardcoded — fallaba cuando data.data era string (`string.message` => `undefined` en JS), cayendo al fallback generico. **Regla: los handlers PHP son inconsistentes en formato de `wp_send_json_error`. Algunos usan string, otros array con `message`. El JS consumer DEBE detectar el formato con `typeof data.data === 'string'` antes de acceder a `.message`. Patron defensivo: `var msg = (typeof data.data === 'string') ? data.data : (data.data.message || data.data || 'fallback');`. Aplicable a ambos branches, success y error.**

3. **`fetch()` NO rechaza la promise con status code HTTP 4xx/5xx — solo rechaza errores de red.** El rate limit retorna HTTP 429, pero `fetch().then(response => response.json())` parsea el body como JSON normalmente. Anti-patron: muchos desarrolladores asumen que un 429/500/403 cae en `.catch()`. En realidad, cae en `.then()` con `data.success === false`. **Regla: el branch de error del JS consumer de fetch DEBE manejar `data.success === false` con el mismo cuidado que el `.catch()` — ahi es donde viven los errores funcionales del server (rate limit, auth fail, validation). El `.catch()` solo captura errores de red (DNS, offline, CORS). Si el JS muestra "Error de conexion" en `.catch()` y el server retorna 429 con JSON valido, el usuario vera "Error de conexion" cuando en realidad el server respondio correctamente con un mensaje actionable.**

4. **Screen readers anuncian grupos de radios por `<legend>` dentro de `<fieldset>`, no por `<label>` cercano.** Los 5 radios `business_type` en form-register.php estaban en un `<div>` con un `<label>` arriba. WCAG 2.1 SC 1.3.1 (Info and Relationships). Sin `<fieldset>` + `<legend>`, NVDA/JAWS/VoiceOver anuncian "radio, Productos fisicos; radio, Productos digitales; ..." sin contexto — el usuario no sabe que todos responden a "¿Qué tipo de productos ofreces?". **Regla: cualquier grupo de radios relacionados DEBE estar en `<fieldset>` con `<legend>` describiendo la pregunta. No es opcional para WCAG AA. Aplicable tambien a checkboxes relacionados (aceptar terminos + acepto comunicaciones comerciales + etc.).**

5. **`opacity:0; pointer-events:none` en radios es INACCESSIBLE para keyboard users.** Patron comun para "custom radio styling" — esconder el radio nativo y dibujar el feedback en el label. PERO `opacity:0` mantiene el radio en el DOM Y en el tab order, sin que el usuario vea donde esta el foco. Cambiar a patron sr-only (`position:absolute; width:1px; height:1px; clip:rect(0,0,0,0); overflow:hidden;`) es mejor: el radio sigue focusable pero no ocupa espacio visual, y `:focus-within` en el label da el feedback visual. **Regla: nunca usar `opacity:0` para esconder inputs focusable. Si el proposito es solo visual (custom styling), `sr-only` pattern + `:focus-within` en el label contenedor. Si el proposito es no-focusable (display management), `display:none` (preferible) o `hidden` attribute.**

6. **`target="_blank"` SIN `rel="noopener noreferrer"` es vulnerabilidad de reverse tabnabbing + UX abrupta.** La pagina abierta en nueva pestana puede hacer `window.opener.location = 'https://phishing.com'` y cuando el usuario vuelve a la pestana original, ve el sitio de phishing creyendo que es el legitimo. Modern browsers (Chrome 88+, Firefox 79+, Safari 12.1+) implicitamente aplican `noopener` cuando `target="_blank"`, PERO no todos los browsers legacy lo hacen, y `noreferrer` (privacidad del referer) requiere explicito. **Regla: TODO `target="_blank"` debe tener `rel="noopener noreferrer"`. Sin excepciones. Aplicar consistente a: links de Terminos, Politica de Privacidad, FAQs, redes sociales, cualquier link externo. Es Lint check obligatorio (regex `/target="_blank"(?!\s+rel=)/` debe retornar 0 matches en lineas de codigo — excluyendo comentarios).**

7. **Labels de opt-in legales ambiguos (SAGRILAFT, GDPR, CCPA) generan friccion sin educar al usuario.** "Autorizo SAGRILAFT (Ley 526 de 1999) *" era un checkbox que el usuario tenia que marcar para continuar el registro, PERO sin saber que es SAGRILAFT ni que autoriza. Opt-in ciego = baja conversion + potencial complain de privacidad despues. Fix: explicacion breve inline ("prevencion de lavado de activos") + link a fuente oficial (funcionpublica.gov.co). Crea confianza Y reduce friccion — el usuario sabe que esta autorizando antes de marcar. **Regla: cualquier opt-in legal/regulatorio DEBE tener (a) explicacion de 1 linea del que se autoriza (no jerga legal), (b) link a fuente oficial del regulatorio/ley, (c) opcionalmente un tooltip o expandable con texto legal completo. Un opt-in ciego es un opt-in que el usuario marca solo para pasar al siguiente step, sin comprender el consentimiento.**

8. **UV-005 `:focus-within` vs `:has(input:focus-visible)` — cobertura cruzada para compat.** `:has()` es mas reciente (Safari 15.4+, Chrome 105+, Firefox 121+) que `:focus-within` (todos los browsers modernos). Para feedback de focus en un label que envuelve un input hidden/sr-only, `:focus-within` es mas compatible. `:has(input:focus-visible)` filtra explicitamente solo focus keyboard (no mouse click). En este fix se usan AMBOS para coverage. **Regla: para inputs sr-only stylists, usar `:focus-within` en el contenedor (label o wrapper div) como baseline, y `:has(input:focus-visible)` para filtering moderno (focus keyboard vs mouse). Aplicar visual feedback consistente (outline + box-shadow) para cumplir WCAG 2.4.7 (Focus Visible).**

9. **Bump `LTMS_VERSION` para cache-busting es obligatorio tras cambios de CSS/JS sin build step.** Sin build step (Webpack/Vite), el cache del browser + SG Optimizer + Cloudflare cachean JS/CSS por URL. Cambiar el contenido del archivo no invalida el cache si la URL es la misma. AGENTS.md: "Si un cambio de CSS/JS no se refleja en dispositivos reales, bumpear `LTMS_VERSION` para forzar cache-busting". En este sub-ciclo se bumpo 2.9.310 → 2.9.311. **Regla: cada commit que modifique cualquier CSS o JS del plugin DEBE bump `LTMS_VERSION`. No esperar a que el usuario reporte "no se ve el cambio" para descubrir que era cache.**

**Reglas preventivas derivadas:**

1. **QA funcional contra server real es complemento obligatorio de auditoria source-based.** Los tests source-based validan estructura (presencia/ausencia de strings, regex). La QA funcional valida comportamiento en runtime con datos reales. UX-001 y UX-002 SOLO fueron detectables porque se hizo POST AJAX real con email duplicado y se observaron responses del server. Source-based solo los hubiera encontrado si se hubiera seguido el flujo E2E mentalmente elemento por elemento — inpractice, los auditores no persiguen cada rama del JS para cada response del handler. QA funcional > source-based para UX bugs.

2. **QA visual con Puppeteer falla en SG por WAF — usar Invoke-WebRequest + UA real como alternativa.** SiteGround WAF bloquea Puppeteer Headless con 403 (Chrome Headless fingerprint). Invoke-WebRequest con UA de Chrome desktop (`Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36`) funciona. Capturar HTML a archivo local y analizar con grep/regex. Para analisis de DOM renderizado en runtime, Puppeteer contra un environment no-WAF (staging o local WP) o browser automation con stealth plugin.

3. **Cleanup obligatorio tras QA funcional que crea usuarios en produccion.** TEST 4 (registro con `admin@example.com`) creo un vendor spurious en produccion (ID=226). Se tuvo que borrar via SSH `wp user delete 226 --yes`. **Regla: toda QA funcional que muta estado (registra usuarios, crea pedidos, envia emails) debe tener un cleanup plan explicito (comando para deshacer) ANTES de ejecutar. Hacer cleanup inmediatamente despues de confirmar el hallazgo, no dejar "para despues".**

4. **Tests source-based con substr buffer son fragiles a fixes futuros del mismo bloque.** AuthReAuditFixTest::test_ra4 tenia buffer 2000 chars para el `else_body` del login handler. UX-004 introdujo 1200 chars de comentarios + `typeof` guard + variable dentro del mismo `else_body`. Resultado: `window.location.href = redirectUrl` que vivio en offset 2221 desde el `else` quedo fuera del buffer 2000. Test falla incluso aunque el fix AUTH-RA4 sigue presente y correcto. **Regla: cuando un test usa `substr(body, else_pos, N)`, N debe tener margen amplio para futuros fix. Mejor: buscar globalmente con `strpos` en lugar de substr — un test que busca "existe `setTimeout` en el bloque del handler" es mas robusto que "existe `setTimeout` dentro de los primeros 2000 chars despues del else". Si se usa substr, documentar el tamaño del bloque actual y dejar 2x margen.**

5. **Cross-checks no-regresion en cada sub-ciclo.** AuthReaUxFixesTest incluye 4 cross-checks validando que los fixes del sub-ciclo RE-AUDIT-AUTH previo (AUTH-RA1..RA4) siguen presentes. Esto evita que un sub-ciclo nuevo deshaga accidentalmente fixes previos. **Regla: cada sub-ciclo nuevo sobre un modulo ya auditado DEBE incluir cross-checks no-regresion contra los fixes del sub-ciclo(s) previo(s) del mismo modulo. Sin esto, un refactor o fix nuevo puede pisar fixes previos y la suitePHPUnit seguira verde (porque el test del sub-ciclo previo valida presencia, no el test nuevo).**

6. **30s antes de comitear: revisar que `php -l` Y `node --check` (si JS) pasen en TODOS los archivos touchados.** Estándar no-negociable. El fix UX-005 toca CSS Y template PHP Y JS — cada uno tiene su propio linter. php -l no atrapa errores de JS, node --check no atrapa errores de PHP. Aplicar los dos a sus respectivos archivos. Sin excepciones.
