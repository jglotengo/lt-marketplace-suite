...
class LTMS_Booking_Season_Manager {
     ...
     * Prioridad: regla de producto > regla global del vendedor.
     *
     * @param int    $product_id
     * @param string $date       Y-m-d
     * @param int    $vendor_id  Necesario para resolver reglas globales con aislamiento multi-tenant.
    ...
    public static function get_modifier_for_date( int $product_id, string $date, int $vendor_id = 0 ): float {
        ...
        // Regla global del vendedor (product_id = 0, vendor_id = X).
        // Si no hay vendor_id, no buscamos globales para evitar fugas multi-tenant.
        if ( $vendor_id > 0 ) {
            $global = $wpdb->get_row(
                $wpdb->prepare(
                    "SELECT price_modifier FROM {$wpdb->prefix}lt_booking_season_rules
    ...
    public static function calculate_total(
        ...
        while ( $current < $end ) {
            ...
            $modifier = self::get_modifier_for_date( $product_id, $date, $vendor_id );
     ...
     *
     * @param array $data Keys: rule_id (opcional), product_id, vendor_id,
     ...
     * @param int $vendor_id ID del vendedor que intenta eliminar.
     * @return bool
     */
    ...
    /**
     * Retorna las reglas de temporada de un producto (incluye globales del mismo vendor).
     *
