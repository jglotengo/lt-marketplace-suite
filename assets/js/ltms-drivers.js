/**
 * LTMS view-drivers — extracted from inline <script>.
...
(function() {
    ...
    function updateKpis() {
        ...
        if (cards[3]) {
            cards[3].textContent = activeRows > 0 ? '<?php echo esc_js( __( "Habilitado", "ltms" ) ); ?>' : '<?php echo esc_js( __( "Deshabilitado", "ltms" ) ); ?>';
    ...
    function openModal(opts) {
        opts = opts || {};
        modalTitle.textContent = opts.edit ? '<?php echo esc_js( __( "Editar repartidor", "ltms" ) ); ?>'
                                            : '<?php echo esc_js( __( "Agregar repartidor", "ltms" ) ); ?>';
        ...
        form.querySelector('[name="driver_document_number"]').placeholder =
            opts.edit ? '<?php echo esc_js( __( "Re-ingresa el documento para confirmar", "ltms" ) ); ?>'
                      : '';
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            submitBtn.textContent = '<?php echo esc_js( __( "Guardando...", "ltms" ) ); ?>';

            fetch(ajaxUrl, { method: 'POST', body: fd, credentials: 'same-origin' })
                .then(function(r) { return r.json(); })
                .then(function(res) {
                    if (res.success) {
                        showToast(res.data.message || '<?php echo esc_js( __( "Repartidor guardado", "ltms" ) ); ?>', 'success');
                        setTimeout(refresh, 800);
                    } else {
                        showToast(res.data || '<?php echo esc_js( __( "Error al guardar", "ltms" ) ); ?>', 'error');
                        submitBtn.disabled = false;
                        submitBtn.textContent = '<?php echo esc_js( __( "Guardar", "ltms" ) ); ?>';
                    }
                })
                .catch(function() {
                    showToast('<?php echo esc_js( __( "Error de red", "ltms" ) ); ?>', 'error');
                    submitBtn.disabled = false;
                    submitBtn.textContent = '<?php echo esc_js( __( "Guardar", "ltms" ) ); ?>';
                });
        });
                ...
                .then(function(res) {
                    if (res.success) {
                        ...
                        if (statusBadge) {
                            ...
                            statusBadge.textContent = isActive ? '<?php echo esc_js( __( "Activo", "ltms" ) ); ?>' : '<?php echo esc_js( __( "Inactivo", "ltms" ) ); ?>';
                        }
                        btn.textContent = isActive ? '<?php echo esc_js( __( "Desactivar", "ltms" ) ); ?>' : '<?php echo esc_js( __( "Activar", "ltms" ) ); ?>';
                        btn.setAttribute('data-active', isActive ? '1' : '0');
                        updateKpis();
                        showToast('<?php echo esc_js( __( "Estado actualizado", "ltms" ) ); ?>', 'success');
                    } else {
                        showToast(res.data || '<?php echo esc_js( __( "Error", "ltms" ) ); ?>', 'error');
                    }
                })
                .catch(function() { showToast('<?php echo esc_js( __( "Error de red", "ltms" ) ); ?>', 'error'); });
        });
    });
                ...
                .then(function(res) {
                    if (res.success) {
                        ...
                        if (badge) {
                            ...
                            badge.textContent = isAvail ? '<?php echo esc_js( __( "Disponible", "ltms" ) ); ?>' : '<?php echo esc_js( __( "Ocupado", "ltms" ) ); ?>';
                        }
                        btn.textContent = isAvail ? '<?php echo esc_js( __( "Marcar ocupado", "ltms" ) ); ?>' : '<?php echo esc_js( __( "Marcar disponible", "ltms" ) ); ?>';
                        btn.setAttribute('data-available', isAvail ? '1' : '0');
                        updateKpis();
                        showToast('<?php echo esc_js( __( "Disponibilidad actualizada", "ltms" ) ); ?>', 'success');
                    } else {
                        showToast(res.data || '<?php echo esc_js( __( "Error", "ltms" ) ); ?>', 'error');
                    }
                })
    ...
    if (confirmDeleteBtn) {
        confirmDeleteBtn.addEventListener('click', function() {
            ...
            confirmDeleteBtn.textContent = '<?php echo esc_js( __( "Eliminando...", "ltms" ) ); ?>';
            fetch(ajaxUrl, { method: 'POST', body: fd, credentials: 'same-origin' })
                .then(function(r) { return r.json(); })
                .then(function(res) {
                    if (res.success) {
                        ...
                        pendingDeleteId = null;
                        updateKpis();
                        showToast('<?php echo esc_js( __( "Repartidor eliminado", "ltms" ) ); ?>', 'success');
                    } else {
                        showToast(res.data || '<?php echo esc_js( __( "Error al eliminar", "ltms" ) ); ?>', 'error');
                    }
                    confirmDeleteBtn.disabled = false;
                    confirmDeleteBtn.textContent = '<?php echo esc_js( __( "Sí, eliminar", "ltms" ) ); ?>';
                })
                .catch(function() {
                    showToast('<?php echo esc_js( __( "Error de red", "ltms" ) ); ?>', 'error');
                    confirmDeleteBtn.disabled = false;
        ...
        settingsForm.addEventListener('submit', function(e) {
            ...
            fetch(ajaxUrl, { method: 'POST', body: fd, credentials: 'same-origin' })
                ...
                .then(function(res) {
                    ...
                    btn.disabled = false;
                    if (res.success) {
                        msg.textContent = res.data || '<?php echo esc_js( __( "Configuración guardada", "ltms" ) ); ?>';
                        msg.style.color = '#16a34a';
                        showToast('<?php echo esc_js( __( "Configuración guardada", "ltms" ) ); ?>', 'success');
                    } else {
                        msg.textContent = res.data || '<?php echo esc_js( __( "Error al guardar", "ltms" ) ); ?>';
                ...
                .catch(function() {
                    spinner.style.display = 'none';
                    btn.disabled = false;
                    msg.textContent = '<?php echo esc_js( __( "Error de red", "ltms" ) ); ?>';
                    msg.style.color = '#dc2626';
                });
        });
    }
