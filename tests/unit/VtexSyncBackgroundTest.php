<?php
/**
 * VtexSyncBackgroundTest — tests de la sync VTEX en background (VTEX-SYNC-BG).
 *
 ...
 *   - ajax_sync_vtex_products(): persiste el filtro de categorías ACTUAL
 *     (CSV o JSON) y programa la sync (NO la ejecuta en el request).
 *   - ajax_sync_vtex_products(): respeta el guard de sync en curso.
 *   - ajax_get_vtex_sync_status(): devuelve el estado para el polling.
 *   - parse_category_ids(): acepta CSV y JSON, descarta no-numéricos.
 *
 * @package LTMS\Tests\Unit
 *
 * Ejecutar con: ./vendor/bin/phpunit --group audit-vtex-background
 *
 * @group audit-vtex-background
 */
...
/**
 * Class VtexSyncBackgroundTest
 *
 * @group audit-vtex-background
 */
final class VtexSyncBackgroundTest extends LTMS_Unit_Test_Case {
	...
	/**
	 * Simula un vendor autenticado con credenciales VTEX válidas.
	 */
	private function stub_logged_vendor( array $meta = [] ): void {
		...
		$defaults = [
			...
			'_ltms_vtex_sync_last_result'   => null,
			'ltms_vtex_last_sync'           => '0',
			'ltms_vtex_last_sync_count'     => '0',
		];
	...
	public function test_get_sync_status_reports_in_progress_and_last_result(): void {
		...
		$this->stub_logged_vendor( [
			'_ltms_vtex_sync_in_progress' => (string) ( $now - 5 ), // <30 min → en curso.
			'_ltms_vtex_sync_last_result' => [
			...
			'ltms_vtex_last_sync_count'   => '10',
		] );

		$status = \LTMS_Vtex_Sync::get_sync_status( 141 );
	...
	public function test_get_sync_status_treats_stale_in_progress_as_not_running(): void {
		...
		$this->stub_logged_vendor( [
			'_ltms_vtex_sync_in_progress' => (string) ( $now - 3600 ), // >30 min → stale (cron matado).
			'_ltms_vtex_sync_last_result' => null,
		] );
	...
	// ─────────────────────────────────────────────────────────────────────────
	// ajax_sync_vtex_products() → programa en background
	// ─────────────────────────────────────────────────────────────────────────
	...
	public function test_ajax_sync_schedules_in_background_and_persists_categories(): void {
		...
		Monkey\Functions\when( 'update_user_meta' )->alias(
			static function ( $user_id, $key, $value, $prev = '' ) use ( $captured ) {
				if ( 'ltms_vtex_category_ids' === $key ) {
					$captured->category_ids = $value;
				}
				if ( 'ltms_vtex_category_ids_csv' === $key ) {
		...
		$response = $this->capture_json_success(
			static fn() => ( new \LTMS_Dashboard_Logic() )->ajax_sync_vtex_products()
		);

		$this->assertTrue( $response['scheduled'] );
		$this->assertSame( 'ltms_vtex_sync_cron', $captured->schedule_args[1], 'Debe programar en el cron hook de VTEX.' );
	...
	public function test_ajax_sync_respects_in_progress_guard(): void {
		...
		$this->stub_logged_vendor( [
			'_ltms_vtex_sync_in_progress' => (string) ( $now - 10 ), // <600s → sync en curso.
		] );
	...
	public function test_ajax_sync_empty_selection_clears_filter(): void {
		$this->require_classes();
		$this->stub_logged_vendor( [
			'ltms_vtex_category_ids_csv' => '8', // Filtro guardado previamente.
		] );
	...
	// ─────────────────────────────────────────────────────────────────────────
	// ajax_get_vtex_sync_status() → polling
	// ─────────────────────────────────────────────────────────────────────────
	...
	public function test_ajax_get_sync_status_returns_status_for_polling(): void {
		...
		$this->stub_logged_vendor( [
			'_ltms_vtex_sync_in_progress' => (string) ( $now - 5 ),
			'_ltms_vtex_sync_last_result' => [ 'completed_at' => '2026-08-31 12:00:00', 'success' => true ],
			'ltms_vtex_last_sync_count'   => '10',
		] );

		$response = $this->capture_json_success(
			static fn() => ( new \LTMS_Dashboard_Logic() )->ajax_get_vtex_sync_status()
		);
	...
	public function test_parse_category_ids_handles_csv_and_json(): void {
		$this->require_classes();
		$instance = new \LTMS_Dashboard_Logic();
		$refl     = new \ReflectionMethod( \LTMS_Dashboard_Logic::class, 'parse_category_ids' );
		$refl->setAccessible( true );

		$this->assertSame( [], $refl->invoke( $instance, '' ) );
		$this->assertSame( [ '8', '4' ], $refl->invoke( $instance, '8,4' ) );
		$this->assertSame( [ '8', '4' ], $refl->invoke( $instance, '["8","4"]' ) );
		$this->assertSame( [ '3' ], $refl->invoke( $instance, '3' ) );
		$this->assertSame( [], $refl->invoke( $instance, 'abc,,1x' ), 'Solo se aceptan ids numéricos.' );
	}
}